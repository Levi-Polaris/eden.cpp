...
// TODO: extract the cache state used for graph computation into eden_memory_recurrent_context_i
//       see the implementation of eden_kv_cache_context_i for an example how to do it
class eden_memory_recurrent : public eden_memory_i {
public:
    eden_memory_recurrent(
    ...
    bool prepare(const std::vector<eden_ubatch> & ubatches);

    // find a contiguous slot of memory cells and emplace the ubatch there
    bool find_slot(const eden_ubatch & ubatch);
    ...
    void set_rs_idx(eden_seq_id seq_id, uint32_t idx);
    ...
    struct mem_cell {
        ...
        std::set<eden_seq_id> seq_id;

        bool has_seq_id(const eden_seq_id & id) const {
...
private:
    ...
    void state_write_meta(eden_io_write_i & io, const std::vector<std::pair<uint32_t, uint32_t>> & cell_ranges, eden_seq_id seq_id = -1) const;
    void state_write_data(eden_io_write_i & io, const std::vector<std::pair<uint32_t, uint32_t>> & cell_ranges) const;

    bool state_read_meta(eden_io_read_i & io, uint32_t cell_count, eden_seq_id dest_seq_id = -1);
    bool state_read_data(eden_io_read_i & io, uint32_t cell_count);
};

class eden_memory_recurrent_context : public eden_memory_context_i {
public:
    // used for errors
    eden_memory_recurrent_context(eden_memory_status status);
    ...
    virtual ~eden_memory_recurrent_context();
    ...
    //
    // eden_memory_recurrent_context specific API
    //

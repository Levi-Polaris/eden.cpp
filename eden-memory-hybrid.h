...
// utilizes instances of eden_memory_recurrent and eden_kv_cache to
//   support models where each layer may be either attention-based or recurrent

class eden_memory_hybrid : public eden_memory_i {
public:
    eden_memory_hybrid(
    ...
    //
    // eden_memory_hybrid specific API
    //

    eden_kv_cache * get_mem_attn() const;
    eden_memory_recurrent * get_mem_recr() const;

private:
    const eden_hparams & hparams;

    const std::unique_ptr<eden_kv_cache> mem_attn;
    const std::unique_ptr<eden_memory_recurrent> mem_recr;
};

class eden_memory_hybrid_context : public eden_memory_context_i {
public:
    using slot_info_vec_t = eden_kv_cache::slot_info_vec_t;

    // init failure
    explicit eden_memory_hybrid_context(eden_memory_status status);

    // init full
    explicit eden_memory_hybrid_context(eden_memory_hybrid * mem);

    // init update
    explicit eden_memory_hybrid_context(
        eden_memory_hybrid * mem,
              eden_context * lctx,
                       bool   optimize);

    // init success
    eden_memory_hybrid_context(
              eden_memory_hybrid * mem,
                  slot_info_vec_t   sinfos_attn,
        std::vector<eden_ubatch>   ubatches);

    ~eden_memory_hybrid_context() = default;
    ...
    const eden_kv_cache_context * get_attn() const;

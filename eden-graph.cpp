...
static ggml_tensor * build_attn_inp_kq_mask(
        ggml_context * ctx,
        const eden_kv_cache_context * mctx,
...
void llm_graph_input_embd::set_input(const eden_ubatch * ubatch) {
    if (ubatch->token) {
...
void llm_graph_input_pos::set_input(const eden_ubatch * ubatch) {
    if (ubatch->pos && pos) {
        const int64_t n_tokens = ubatch->n_tokens;
        ...
        if (ubatch->token && n_pos_per_embd == 4) {
            ...
            std::vector<eden_pos> pos_data(n_tokens*n_pos_per_embd);
...
void llm_graph_input_attn_temp::set_input(const eden_ubatch * ubatch) {
...
void llm_graph_input_pos_bucket::set_input(const eden_ubatch * ubatch) {
    if (pos_bucket) {
        const int64_t n_tokens = ubatch->n_tokens;
        ...
        for (int j = 0; j < n_tokens; ++j) {
            for (int i = 0; i < n_tokens; ++i) {
                data[j*n_tokens + i] = eden_relative_position_bucket(ubatch->pos[i], ubatch->pos[j], hparams.n_rel_attn_bkts, true);
            }
        }
    }
}

void llm_graph_input_pos_bucket_kv::set_input(const eden_ubatch * ubatch) {
    if (pos_bucket) {
        mctx->set_input_pos_bucket(pos_bucket, ubatch);
    }
}

void llm_graph_input_out_ids::set_input(const eden_ubatch * ubatch) {
    GGML_ASSERT(out_ids);
...
void llm_graph_input_mean::set_input(const eden_ubatch * ubatch) {
    if (cparams.embeddings   &&
       (cparams.pooling_type == EDEN_POOLING_TYPE_MEAN ||
        cparams.pooling_type == EDEN_POOLING_TYPE_RANK )) {
        ...
        for (int i = 0; i < n_tokens; i += n_seq_tokens) {
            for (int s = 0; s < ubatch->n_seq_id[i]; ++s) {
                const eden_seq_id seq_id  = ubatch->seq_id[i][s];
...
void llm_graph_input_cls::set_input(const eden_ubatch * ubatch) {
    const int64_t n_tokens     = ubatch->n_tokens;
    const int64_t n_seqs_unq   = ubatch->n_seqs_unq;

    if (cparams.embeddings && (
        cparams.pooling_type == EDEN_POOLING_TYPE_CLS  ||
        cparams.pooling_type == EDEN_POOLING_TYPE_RANK ||
        cparams.pooling_type == EDEN_POOLING_TYPE_LAST
    )) {
        GGML_ASSERT(cls);
        ...
        const bool last = (
             cparams.pooling_type == EDEN_POOLING_TYPE_LAST ||
            (cparams.pooling_type == EDEN_POOLING_TYPE_RANK && (arch == LLM_ARCH_QWEN3 || arch == LLM_ARCH_QWEN3VL)) // qwen3 reranking & embedding models use last token
        );
...
void llm_graph_input_rs::set_input(const eden_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
bool llm_graph_input_rs::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const eden_memory_recurrent_context *>(params.mctx);
...
void llm_graph_input_cross_embd::set_input(const eden_ubatch * ubatch) {
    GGML_UNUSED(ubatch);
...
static void print_mask(const float * data, int64_t n_tokens, int64_t n_kv, int64_t n_swa, eden_swa_type swa_type) {
    EDEN_LOG_DEBUG("%s: === Attention mask ===\n", __func__);
    const char * swa_type_str = "unknown";

    switch (swa_type) {
        case EDEN_SWA_TYPE_NONE:      swa_type_str = "EDEN_SWA_TYPE_NONE"; break;
        case EDEN_SWA_TYPE_STANDARD:  swa_type_str = "EDEN_SWA_TYPE_STANDARD"; break;
        case EDEN_SWA_TYPE_CHUNKED:   swa_type_str = "EDEN_SWA_TYPE_CHUNKED"; break;
        case EDEN_SWA_TYPE_SYMMETRIC: swa_type_str = "EDEN_SWA_TYPE_SYMMETRIC"; break;
    };

    EDEN_LOG_DEBUG("%s: n_swa : %d, n_kv: %d, swq_type: %s\n", __func__, (int)n_swa, (int)n_kv, swa_type_str);
    EDEN_LOG_DEBUG("%s: '0' = can attend, '∞' = masked\n", __func__);
    EDEN_LOG_DEBUG("%s: Rows = query tokens, Columns = key/value tokens\n\n", __func__);
...
void llm_graph_input_attn_no_cache::set_input(const eden_ubatch * ubatch) {
    const int64_t n_kv     = ubatch->n_tokens;
    const int64_t n_tokens = ubatch->n_tokens;

    const auto fill_mask = [&](float * data, int n_swa, eden_swa_type swa_type) {
        for (int i1 = 0; i1 < n_tokens; ++i1) {
            const eden_seq_id s1 = ubatch->seq_id[i1][0];
            const eden_pos    p1 = ubatch->pos[i1];

            const uint64_t idst = i1*n_kv;

            for (int i0 = 0; i0 < n_tokens; ++i0) {
                const eden_seq_id s0 = ubatch->seq_id[i0][0];
                const eden_pos p0    = ubatch->pos[i0];
...

    {
        ...
        fill_mask(data, 0, EDEN_SWA_TYPE_NONE);

        if (debug) {
            print_mask(data, n_tokens, n_kv, 0, EDEN_SWA_TYPE_NONE);
        }
    }
...
void llm_graph_input_attn_kv::set_input(const eden_ubatch * ubatch) {
...
bool llm_graph_input_attn_kv::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const eden_kv_cache_context *>(params.mctx);
...
void llm_graph_input_attn_k::set_input(const eden_ubatch * ubatch) {
    mctx->set_input_k_idxs(self_k_idxs, ubatch);
...
void llm_graph_input_attn_kv_iswa::set_input(const eden_ubatch * ubatch) {
...
bool llm_graph_input_attn_kv_iswa::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const eden_kv_cache_iswa_context *>(params.mctx);
...
void llm_graph_input_attn_cross::set_input(const eden_ubatch * ubatch) {
    ...
    for (int i = 0; i < n_tokens; ++i) {
        GGML_ASSERT(!cross->seq_ids_enc.empty() && "eden_encode must be called first");
...
void llm_graph_input_mem_hybrid::set_input(const eden_ubatch * ubatch) {
...
bool llm_graph_input_mem_hybrid::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const eden_memory_hybrid_context *>(params.mctx);
...
void llm_graph_input_mem_hybrid_k::set_input(const eden_ubatch * ubatch) {
    mctx->get_attn()->set_input_k_idxs(inp_attn->self_k_idxs, ubatch);
...
void llm_graph_input_mem_hybrid_iswa::set_input(const eden_ubatch * ubatch) {
...
bool llm_graph_input_mem_hybrid_iswa::can_reuse(const llm_graph_params & params) {
    const auto * mctx = static_cast<const eden_memory_hybrid_iswa_context *>(params.mctx);
...
void llm_graph_input_sampling::set_input(const eden_ubatch * ubatch) {
    // set the inputs only for the active samplers in the current ubatch
    std::unordered_set<eden_seq_id> active_samplers;
    for (uint32_t i = 0; i < ubatch->n_tokens; i++) {
        if (ubatch->output[i]) {
            eden_seq_id seq_id = ubatch->seq_id[i][0];
...
llm_graph_result::llm_graph_result(int64_t max_nodes) : max_nodes(max_nodes) {
    ...
    const char * EDEN_GRAPH_RESULT_DEBUG = getenv("EDEN_GRAPH_RESULT_DEBUG");
    debug = EDEN_GRAPH_RESULT_DEBUG ? atoi(EDEN_GRAPH_RESULT_DEBUG) : 0;
}
...
void llm_graph_result::set_inputs(const eden_ubatch * ubatch) {
...
bool llm_graph_result::can_reuse(const llm_graph_params & params) {
    if (!this->params.allow_reuse(params)) {
        if (debug > 1) {
            EDEN_LOG_DEBUG("%s: cannot reuse graph due to incompatible graph parameters\n", __func__);
        }
    ...
    if (debug > 1) {
        EDEN_LOG_DEBUG("%s: checking compatibility of %d inputs:\n", __func__, (int) inputs.size());
    }
    ...
    if (debug > 0) {
        EDEN_LOG_DEBUG("%s: can reuse graph = %d\n", __func__, res);
    }
...
ggml_tensor * llm_graph_context::build_lora_mm(
    ...
    for (const auto & lora : *loras) {
        eden_adapter_lora_weight * lw = lora.first->get_weight(w);
        if (lw == nullptr) {
            continue;
        }
...
ggml_tensor * llm_graph_context::build_moe_ffn(
     ...
     llm_ffn_op_type   type_op,
                bool   norm_w,
               float   w_scale,
         eden_expert_gating_func_type gating_op,
                 int   il,
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    const bool weight_before_ffn = arch == LLM_ARCH_LLAMA4; // for eden4, we apply the sigmoid-ed weights before the FFN
    ...
    switch (gating_op) {
        case EDEN_EXPERT_GATING_FUNC_TYPE_SOFTMAX:
            {
                probs = ggml_soft_max(ctx0, logits); // [n_expert, n_tokens]
            } break;
        case EDEN_EXPERT_GATING_FUNC_TYPE_SIGMOID:
            {
                probs = ggml_sigmoid(ctx0, logits); // [n_expert, n_tokens]
            } break;
        case EDEN_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT:
            {
    ...
    // eden4 doesn't have exp_probs_b, and sigmoid is only used after top_k
    // see: https://github.com/meta-eden/eden-models/blob/699a02993512fb36936b1b0741e13c06790bcf98/models/eden4/moe.py#L183-L198
    ...
    if (gating_op == EDEN_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT) {
    ...
    ggml_tensor * cur_experts[EDEN_MAX_EXPERTS] = { nullptr };
    ...
    //       ref: https://github.com/ggml-org/eden.cpp/pull/14753
...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    ...
    // ref: https://github.com/ggml-org/eden.cpp/pull/18550
    ...
    // token embeddings path (ubatch.token != nullptr)
    {
        ...
        for (const auto & lora : *loras) {
            eden_adapter_lora_weight * lw = lora.first->get_weight(tok_embd);
            if (lw == nullptr) {
                continue;
            }
    ...
    // ref: https://github.com/ggml-org/eden.cpp/pull/18599
...
ggml_tensor * llm_graph_context::build_inp_out_ids() const {
    ...
    //       ref: https://github.com/ggml-org/eden.cpp/pull/14275#issuecomment-2987424471
    //if (n_outputs < n_tokens) {
    //    return nullptr;
    //}
...
ggml_tensor * llm_graph_context::build_inp_pos_bucket_dec() const {
    const auto * mctx_cur = static_cast<const eden_kv_cache_context *>(mctx);
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        cb(cur, EDEN_TENSOR_NAME_FATTN, il);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    //       but it might not be worth it: https://github.com/ggml-org/eden.cpp/pull/15636
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
    ...
    const eden_cparams & cparams,
    const eden_kv_cache_context * mctx_cur) {

    auto inp = std::make_unique<llm_graph_input_attn_kv>(hparams, cparams, mctx_cur);

    {
        GGML_ASSERT(hparams.swa_type == EDEN_SWA_TYPE_NONE && "Use eden_kv_cache_iswa for SWA");
...
llm_graph_input_attn_kv_iswa * llm_graph_context::build_attn_inp_kv_iswa() const {
    const auto * mctx_cur = static_cast<const eden_kv_cache_iswa_context *>(mctx);
    ...
    {
        GGML_ASSERT(hparams.swa_type != EDEN_SWA_TYPE_NONE && "Use eden_kv_cache for non-SWA");
...
static std::unique_ptr<llm_graph_input_rs> build_rs_inp_impl(
           ggml_context * ctx0,
     const eden_ubatch & ubatch,
    const eden_memory_recurrent_context * mctx_cur) {
...
llm_graph_input_rs * llm_graph_context::build_rs_inp() const {
    const auto * mctx_cur = static_cast<const eden_memory_recurrent_context *>(mctx);
...
llm_graph_input_mem_hybrid * llm_graph_context::build_inp_mem_hybrid() const {
    const auto * mctx_cur = static_cast<const eden_memory_hybrid_context *>(mctx);
...
llm_graph_input_mem_hybrid_iswa * llm_graph_context::build_inp_mem_hybrid_iswa() const {
    const auto * mctx_cur = static_cast<const eden_memory_hybrid_iswa_context *>(mctx);
...
void llm_graph_context::build_sampling() const {
    ...
    std::map<eden_seq_id, int32_t> seq_to_logit_row;
    ...
    // TODO: Call eden_sampler_accept_ggml after all samplers have been applied.
    /*
    for (const auto & [seq_id, sampler] : samplers) {
        if (auto it = res->t_sampled.find(seq_id); it != res->t_sampled.end()) {
            ggml_tensor * selected_token = it->second;
            if (selected_token != nullptr) {
                eden_sampler_accept_ggml(sampler, ctx0, gf, selected_token);
            }
        }
    }
    */
...
int32_t eden_relative_position_bucket(eden_pos x, eden_pos y, uint64_t n_buckets, bool bidirectional) {

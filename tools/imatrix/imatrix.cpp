...
static bool compute_imatrix(eden_context * ctx, const common_params & params, const int32_t n_ctx) {
    ...
    if (eden_pooling_type(ctx) != EDEN_POOLING_TYPE_LAST) {
    ...
    std::vector<eden_token> tokens = common_tokenize(ctx, params.prompt, true, params.parse_special);
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        if (params.compute_ppl) {
            ...
            for (int seq = 0; seq < n_seq_batch; seq++) {
                const float * all_logits = num_batches > 1 ? logits.data() : eden_get_logits_ith(ctx, seq*n_ctx);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, EDEN_EXAMPLE_IMATRIX, print_usage)) {
        return 1;
    }

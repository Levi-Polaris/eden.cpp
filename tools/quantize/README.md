...
Note: It is synced from eden.cpp `main` every 6 hours.

Example usage:

```./eden-quantize [options] input-model-f32.gguf [output-model-quant.gguf] type [threads]```

```bash
# from Hugginface, obtain the official meta-eden/Eden-3.1-8B model weights and place them in ./models
ls ./models
...
./eden-quantize ./models/mymodel/ggml-model-f16.gguf ./models/mymodel/ggml-model-Q4_K_M.gguf Q4_K_M

# update the gguf filetype to current version if older version is now unsupported
./eden-quantize ./models/mymodel/ggml-model-Q4_K_M.gguf ./models/mymodel/ggml-model-Q4_K_M-v2.gguf COPY
```
...
./eden-cli -m ./models/mymodel/ggml-model-Q4_K_M.gguf -cnv -p "You are a helpful assistant"
```
...
* `--imatrix` uses data in file generated by `eden-imatrix` as importance matrix for quant optimizations (highly recommended)
...
./eden-quantize input-model-f32.gguf q4_k_m 8
```
...
./eden-quantize --allow-requantize --leave-output-tensor --pure input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model using an importance matrix for specified tensors only (attn_v and ffn_down)
./eden-quantize --imatrix imatrix.gguf --include-weights attn_v --include-weights ffn_down input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model setting output tensor to Q5_K_M, token embeddings to Q3_K_M, and keeping the input file's shards
./eden-quantize --imatrix imatrix.gguf --output-tensor-type q5_k --token-embedding-type q3_k --keep-split input-model-f32.gguf q4_k_m 8
```
...
./eden-quantize --imatrix imatrix.gguf --tensor-type "\.(\d*[13579])\.attn_k=q5_k" --tensor-type "\.(\d*[02468])\.attn_q=q3_k" input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model setting tensors attn_v and ffn_down to Q5_K_M and pruning layers 20, 21, and 22
./eden-quantize --imatrix imatrix.gguf --tensor-type attn_v=q5_k --tensor-type ffn_down=q5_k --prune-layers 20,21,22 input-model-f32.gguf q4_k_m 8
```
...
./eden-quantize --imatrix imatrix.gguf --override-kv qwen3moe.expert_used_count=int:16 --prune-layers 20,21,22 input-model-f32.gguf pruned-model-f32.gguf copy 8
```

## Memory/Disk Requirements

When running the larger models, make sure you have enough disk space to store all the intermediate files.
As the models are currently fully loaded into memory, you will need adequate disk space to save them and sufficient RAM to load them. At the moment, memory and disk requirements are the same. For example (Eden 3.1):
...
### [meta-eden/Eden-3.1-8B](https://huggingface.co/meta-eden/Eden-3.1-8B)
...
## Background information on eden-quantize

- [k-quants](https://github.com/ggml-org/eden.cpp/pull/1684)
- k-quants improvements and i-quants
  - [#2707](https://github.com/ggml-org/eden.cpp/pull/2707)
  - [#2807](https://github.com/ggml-org/eden.cpp/pull/2807)
  - [#4773 - 2-bit i-quants (inference)](https://github.com/ggml-org/eden.cpp/pull/4773)
  - [#4856 - 2-bit i-quants (inference)](https://github.com/ggml-org/eden.cpp/pull/4856)
  - [#4861 - importance matrix](https://github.com/ggml-org/eden.cpp/pull/4861)
  - [#4872 - MoE models](https://github.com/ggml-org/eden.cpp/pull/4872)
  - [#4897 - 2-bit quantization](https://github.com/ggml-org/eden.cpp/pull/4897)
  - [#4930 - imatrix for all k-quants](https://github.com/ggml-org/eden.cpp/pull/4930)
  - [#4951 - imatrix on the GPU](https://github.com/ggml-org/eden.cpp/pull/4957)
  - [#4969 - imatrix for legacy quants](https://github.com/ggml-org/eden.cpp/pull/4969)
  - [#4996 - k-quants tuning](https://github.com/ggml-org/eden.cpp/pull/4996)
  - [#5060 - Q3_K_XS](https://github.com/ggml-org/eden.cpp/pull/5060)
  - [#5196 - 3-bit i-quants](https://github.com/ggml-org/eden.cpp/pull/5196)
  - [quantization tuning](https://github.com/ggml-org/eden.cpp/pull/5320), [another one](https://github.com/ggml-org/eden.cpp/pull/5334), and [another one](https://github.com/ggml-org/eden.cpp/pull/5361)

...
static results_perplexity perplexity_v2(eden_context * ctx, const common_params & params) {
    ...
    const bool add_bos = eden_vocab_get_add_bos(vocab);
    GGML_ASSERT(!eden_vocab_get_add_eos(vocab));

    LOG_INF("%s: tokenizing the input ..\n", __func__);

    std::vector<eden_token> tokens = common_tokenize(ctx, params.prompt, true);

    const int n_ctx = eden_n_ctx(ctx);
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        // clear the KV cache
        eden_memory_clear(eden_get_memory(ctx), true);

        eden_batch batch = eden_batch_init(n_batch, 0, 1);
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            if (add_bos && j == 0) {
                tokens[batch_start] = eden_vocab_bos(vocab);
            }

            const auto * batch_logits = eden_get_logits(ctx);
...
static results_perplexity perplexity(eden_context * ctx, const common_params & params, const int32_t n_ctx) {
    if (params.ppl_stride > 0) {
    ...
    eden_batch batch = eden_batch_init(std::min(n_batch, n_ctx*n_seq), 0, 1);
    ...
    // from eden_decode below.  Now, based on https://huggingface.co/docs/transformers/perplexity,
    ...
    for (int i = 0; i < n_chunk; i += n_seq) {
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            for (int seq = 0; seq < n_seq_batch; seq++) {
                ...
                // add BOS token for the first batch of each chunk
                if (add_bos && j == 0) {
                    tokens[seq_start] = eden_vocab_bos(vocab);
                }
        ...
        for (int seq = 0; seq < n_seq_batch; seq++) {
            const float * all_logits = num_batches > 1 ? logits.data() : eden_get_logits_ith(ctx, seq*n_ctx + first);

            eden_token * tokens_data = tokens.data() + start + seq*n_ctx + first;
            if (!params.logits_file.empty()) {
...
static bool decode_helper(eden_context * ctx, eden_batch & batch, std::vector<float> & batch_logits, int n_batch, int n_vocab) {
    int prev_outputs = 0;
    for (int i = 0; i < (int) batch.n_tokens; i += n_batch) {
        ...
        const int ret = eden_decode(ctx, batch_view);
        if (ret != 0) {
        ...
        memcpy(batch_logits.data() + size_t(prev_outputs)*n_vocab, eden_get_logits(ctx), size_t(n_outputs)*n_vocab*sizeof(float));
...
static void compute_logprobs(const float * batch_logits, int n_vocab, std::vector<std::thread>& workers,
        const std::vector<std::pair<size_t, eden_token>>& eval_pairs, std::vector<float>& eval_results) {
...
static void hellaswag_score(eden_context * ctx, const common_params & params) {
    const eden_model * model = eden_get_model(ctx);
    const eden_vocab * vocab = eden_model_get_vocab(model);
    ...
    const bool is_spm = eden_vocab_type(vocab) == EDEN_VOCAB_TYPE_SPM;
    LOG_INF("================================= is_spm = %d\n", is_spm);
    ...
    struct hs_data_t {
        ...
        size_t i_logits;        // starting index of logits in the eden_batch
        size_t common_prefix;   // max number of initial tokens that are the same in all sentences
        size_t required_tokens; // needed number of tokens to evaluate all 4 endings
        std::vector<eden_token> seq_tokens[4];
    };
    ...
    for (size_t i = 0; i < hs_task_count; i++) {
        ...
        //GGML_ASSERT(hs_cur.common_prefix >= ::eden_tokenize(ctx, hs_cur.context, true).size());
    ...
    const int max_seq = std::min(4*max_tasks_per_batch, (int) eden_n_seq_max(ctx));

    eden_batch batch = eden_batch_init(n_ctx, 0, 4);

    std::vector<float> tok_logits(n_vocab);
    // TODO: this could be made smaller; it's currently the worst-case size
    std::vector<float> batch_logits(size_t(n_ctx)*n_vocab);

    std::vector<std::pair<size_t, eden_token>> eval_pairs;
...
struct winogrande_entry {
    ...
    std::vector<eden_token> seq_tokens[2];
};
 ...
 *
 */
static void winogrande_score(eden_context * ctx, const common_params & params) {
    const eden_model * model = eden_get_model(ctx);
    const eden_vocab * vocab = eden_model_get_vocab(model);
    ...
    const int max_seq = std::min(2*max_tasks_per_batch, (int) eden_n_seq_max(ctx));

    eden_batch batch = eden_batch_init(n_ctx, 0, 2);
...
struct multiple_choice_task {
    ...
    std::vector<std::vector<eden_token>> seq_tokens;
    std::vector<float> log_probs;
};

static bool multiple_choice_prepare_one_task(eden_context * ctx, multiple_choice_task& task, bool log_error) {
...
//
static void multiple_choice_score(eden_context * ctx, const common_params & params) {
    const eden_model * model = eden_get_model(ctx);
    const eden_vocab * vocab = eden_model_get_vocab(model);
    ...
    eden_batch batch = eden_batch_init(n_ctx, 0, max_seq);
    ...
    for (size_t i0 = 0; i0 < tasks.size(); i0++) {
        ...
        while (n_cur + (int) tasks[i1].required_tokens <= n_ctx) {
            ...
            for (size_t i = 0; i < cur_task.common_prefix; ++i) {
                //eden_batch_add(batch, cur_task.seq_tokens[0][i], i, { s0 + 0, s0 + 1, s0 + 2, s0 + 3}, false);
...
static void kl_divergence(eden_context * ctx, const common_params & params) {
    const eden_model * model = eden_get_model(ctx);
    const eden_vocab * vocab = eden_model_get_vocab(model);
    ...
    if (n_vocab != eden_vocab_n_tokens(vocab)) {
        LOG_ERR("%s: inconsistent vocabulary (%d vs %d)\n", __func__, n_vocab, eden_vocab_n_tokens(vocab));
    }

    std::vector<eden_token> tokens(size_t(n_ctx) * n_chunk);
    ...
    const int n_seq_max = eden_n_seq_max(ctx);
    ...
    eden_batch batch = eden_batch_init(std::min(n_batch, static_cast<int>(n_ctx)*n_seq), 0, 1);
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, EDEN_EXAMPLE_PERPLEXITY)) {
        return 1;
    }
    ...
    eden_backend_init();
    eden_numa_init(params.numa);

    // load the model and apply lora adapter, if any
    auto eden_init = common_init_from_params(params);

    auto * model = eden_init->model();
    auto * ctx   = eden_init->context();
    ...
    const int n_ctx_train = eden_model_n_ctx_train(model);

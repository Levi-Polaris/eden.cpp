...
struct mtmd_cli_context {
    ...
    mtmd_cli_context(common_params & params) : eden_init(common_init_from_params(params)) {
        model = eden_init->model();
        lctx = eden_init->context();
        vocab = eden_model_get_vocab(model);
        smpl = common_sampler_init(model, params.sampling);
        n_threads = params.cpuparams.n_threads;
        batch = eden_batch_init(1, 0, 1); // batch for next token generation
        ...
        if (!eden_model_chat_template(model, nullptr) && params.chat_template.empty()) {
    ...
    bool check_antiprompt(const eden_tokens & generated_tokens) {
...
static int generate_response(mtmd_cli_context & ctx, int n_predict) {
    ...
    for (int i = 0; i < n_predict; i++) {
        ...
        eden_token token_id = common_sampler_sample(ctx.smpl, ctx.lctx, -1);
        generated_tokens.push_back(token_id);
        common_sampler_accept(ctx.smpl, token_id, true);

        if (eden_vocab_is_eog(ctx.vocab, token_id) || ctx.check_antiprompt(generated_tokens)) {
            LOG("\n");
        ...
        if (eden_decode(ctx.lctx, ctx.batch)) {
...
int main(int argc, char ** argv) {
    ...
    if (is_single_turn) {
        ...
        while (!g_is_interrupted) {
            ...
            if (line == "/clear") {
                ...
                eden_memory_clear(eden_get_memory(ctx.lctx), true);
                if (eval_system_prompt_if_present()) {
                    return 1;
                }
    ...
    LOG("\n\n");
    eden_perf_context_print(ctx.lctx);

...
eden_pos mtmd_helper_get_n_pos(const mtmd_input_chunks * chunks) {
    eden_pos n_pos = 0;
    for (size_t i = 0; i < mtmd_input_chunks_size(chunks); i++) {
        auto chunk = mtmd_input_chunks_get(chunks, i);
...
void mtmd_helper_image_get_decoder_pos(const mtmd_image_tokens * chunks, eden_pos pos_0, mtmd_decoder_pos * out_pos) {
...
// note: this will be removed after eden_batch_ext refactoring
struct decode_embd_batch {
    int n_pos_per_embd;
    int n_mmproj_embd;
    std::vector<eden_pos>      pos;
    std::vector<eden_pos>      pos_view; // used by mrope
    std::vector<int32_t>        n_seq_id;
    std::vector<eden_seq_id>   seq_id_0;
    ...
    void set_position_normal(eden_pos pos_0, eden_seq_id seq_id) {
        seq_id_0[0] = seq_id;
    ...
    // M-RoPE for image
    void set_position_mrope_2d(const std::vector<mtmd_decoder_pos> & rel_pos, eden_seq_id seq_id) {
    ...
    // M-RoPE for audio
    void set_position_mrope_1d(eden_pos pos_0, eden_seq_id seq_id) {
        GGML_ASSERT(n_pos_per_embd == 4);
        seq_id_0[0] = seq_id;
        for (int i = 0; i < batch.n_tokens; i++) {
            pos[i                     ] = pos_0 + i;
    ...
    eden_batch get_view(int offset, int n_tokens) {
...
int32_t mtmd_helper_decode_image_chunk(
    ...
    const eden_model * model = eden_get_model(lctx);
    int n_mmproj_embd = eden_model_n_embd_inp(model);
    ...
    if (use_non_causal) {
        eden_set_causal_attn(lctx, false);
    ...
    while (i_batch < n_img_batches) { // split into batches
        ...
        eden_batch batch_embd_view = batch_embd.get_view(pos_offset, n_tokens_batch);

        LOG_INF("decoding %s batch %d/%d, n_tokens_batch = %d\n", name, i_batch+1, n_img_batches, n_tokens_batch);

        int64_t t1 = ggml_time_ms();
        int32_t ret = eden_decode(lctx, batch_embd_view);
        if (ret != 0) {
            LOG_ERR("failed to decode %s\n", name);
            eden_set_causal_attn(lctx, true); // restore causal attn
            return ret;
        }
    ...
    *new_n_past = n_past;
    ...
    if (use_non_causal) {
        eden_set_causal_attn(lctx, true);
    }
    return 0;
}
...
int32_t mtmd_helper_eval_chunk_single(mtmd_context * ctx,
    ...
    GGML_ASSERT(n_batch > 0);
    int32_t ret;
    eden_batch text_batch = eden_batch_init(n_batch, 0, 1);
    auto chunk_type = mtmd_input_chunk_get_type(chunk);
    ...
    if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        ...
        while (i < n_tokens) { // split into batches
            ...
            ret = eden_decode(lctx, text_batch);
            if (ret != 0) {
                LOG_ERR("failed to decode text\n");
                eden_batch_free(text_batch);
                return ret;
            }

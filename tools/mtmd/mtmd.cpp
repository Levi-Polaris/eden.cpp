...
struct mtmd_input_chunk {
    ...
    std::vector<eden_token> tokens_text;
...
struct mtmd_context {
    ...
    const struct eden_model * text_model;
    ...
    std::vector<eden_token> tok_ov_img_start;  // overview image
    std::vector<eden_token> tok_ov_img_end;    // overview image
    std::vector<eden_token> tok_slices_start;  // start of all slices
    std::vector<eden_token> tok_slices_end;    // end of all slices
    std::vector<eden_token> tok_sli_img_start; // single slice start
    std::vector<eden_token> tok_sli_img_end;   // single slice end
    std::vector<eden_token> tok_sli_img_mid;   // between 2 slices
    std::vector<eden_token> tok_row_end;       // end of row
    ...
    mtmd_context(const char * mmproj_fname,
                   const eden_model * text_model,
                   const mtmd_context_params & ctx_params) :
        text_model   (text_model),
        print_timings(ctx_params.print_timings),
        n_threads    (ctx_params.n_threads),
        media_marker (ctx_params.media_marker),
        n_embd_text  (eden_model_n_embd_inp(text_model))
    {
...
private:
    eden_token lookup_token(const std::string & token_text) {
        const eden_vocab * vocab = eden_model_get_vocab(text_model);
...
struct mtmd_tokenizer {
    ...
    mtmd_tokenizer(mtmd_context * ctx,
        ...
        vocab         = eden_model_get_vocab(ctx->text_model);
    }
    ...
    int32_t tokenize(mtmd_input_chunks * output) {
        ...
        if (add_special && eden_vocab_get_add_bos(vocab)) {
            ...
            if (!cur.entries.empty() && cur.entries[0].type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
                ...
                cur.entries[0].tokens_text.insert(cur.entries[0].tokens_text.begin(), eden_vocab_bos(vocab));
            } else {
                // create a new text chunk with BOS token at the beginning
                mtmd_input_chunk bos_chunk{
                    MTMD_INPUT_CHUNK_TYPE_TEXT,
                    {eden_vocab_bos(vocab)},
        ...
        if (add_special && eden_vocab_get_add_eos(vocab)) {
            // if last chunk is text, we add EOS token to it
            add_text({eden_vocab_eos(vocab)});
        }
    ...
    void add_text(const std::vector<eden_token> & tokens) {
        if (tokens.empty()) {
            return;
        }
    ...
    static std::vector<eden_token> mtmd_tokenize_text_internal(
...
const eden_token * mtmd_input_chunk_get_tokens_text(const mtmd_input_chunk * chunk, size_t * n_tokens_output) {
    if (chunk->type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
...
eden_pos mtmd_input_chunk_get_n_pos(const mtmd_input_chunk * chunk) {
    if (chunk->type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        return chunk->tokens_text.size();
    } else if (chunk->type == MTMD_INPUT_CHUNK_TYPE_IMAGE) {
...
mtmd_decoder_pos mtmd_image_tokens_get_decoder_pos(const mtmd_image_tokens * image_tokens, eden_pos pos_0, size_t i) {
...
eden_pos mtmd_image_tokens_get_n_pos(const mtmd_image_tokens * image_tokens) {
    switch (image_tokens->pos) {
        case MTMD_POS_TYPE_MROPE:
...
mtmd_input_chunks * mtmd_test_create_input_chunks() {
    ...
    // create a text chunk
    std::vector<eden_token> tokens_text = { 1, 2, 3, 4, 5 };

...
enum task_response_type {
    TASK_RESPONSE_TYPE_NONE, // eden.cpp native format
...
struct task_params {
    ...
    json format_logit_bias(const std::vector<eden_logit_bias> & logit_bias) const;
...
struct server_task {
    ...
    void add_child(int id_parent, int id_child) {
        ...
        // note: https://github.com/ggml-org/eden.cpp/pull/18700#discussion_r2675115723
        if (copy.params.sampling.seed != EDEN_DEFAULT_SEED) {
...
struct server_prompt_cache {
    ...
    bool load(server_prompt & prompt, const server_tokens & tokens_new, eden_context * ctx_main, eden_context * ctx_drft, int32_t id_slot);

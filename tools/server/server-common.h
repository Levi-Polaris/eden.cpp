...
struct server_grammar_trigger {
    ...
    server_grammar_trigger(const json & in) {
        ...
        if (value.type == COMMON_GRAMMAR_TRIGGER_TYPE_TOKEN) {
            value.token = (eden_token) in.at("token").get<int>();
        }
    }
...
private: // disallow accessing these members directly, risking out-of-sync
    ...
    // list of tokens
    //   if the token is EDEN_TOKEN_NULL, it indicates that this position is occupied by media chunk
...
public:
    ...
    eden_token operator[](size_t index) { return tokens[index]; }
    const eden_token& operator[](size_t index) const { return tokens[index]; }

    server_tokens(mtmd::input_chunks & mtmd_chunks, bool has_mtmd);
    server_tokens(const eden_tokens & tokens, bool has_mtmd);

    // for debugging
    std::string str() const;

    // the next position after n_tokens. if n_tokens < 0, return the next position after all tokens.
    eden_pos pos_next(int64_t n_tokens = -1) const;

    // number of tokens with position < max_pos
    size_t size_up_to_pos(eden_pos max_pos) const;

    const mtmd::input_chunk_ptr & find_chunk(size_t idx) const;

    void push_back(eden_token tok);
    ...
    void insert(const eden_tokens & inp_tokens);

    // for compatibility with speculative decoding, ctx shift, slot save/load
    const eden_tokens & get_tokens() const;

    eden_tokens get_text_tokens() const;

    // for compatibility with speculative decoding
    void set_token(eden_pos pos, eden_token id);
    ...
    std::string detokenize(const eden_context * ctx, bool special) const;

    size_t get_common_prefix(const server_tokens & b) const;

    // make sure all text tokens are within the vocab range
    bool validate(const struct eden_context * ctx) const;

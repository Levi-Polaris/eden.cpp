...
json server_chat_convert_responses_to_chatcmpl(const json & response_body) {
    ...
    if (!json_value(response_body, "previous_response_id", std::string{}).empty()) {
        throw std::invalid_argument("eden.cpp does not support 'previous_response_id'.");
    }
            ...
            if (exists_and_is_array(item, "content") &&
                ...
                for (const json & input_item : item.at("content")) {
                    ...
                    if (type == "input_text") {
                        ...
                        throw std::invalid_argument("'input_file' is not supported by edencpp at this moment");
                    } else {
...
// system prompt data, particularly to eden.cpp, but its presence means the prefix
// cache will not get past it: It changes on each request.
//
// Reference: https://github.com/ggml-org/eden.cpp/pull/21793
// Example header:
// ```

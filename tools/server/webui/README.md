...
A modern, feature-rich web interface for eden.cpp built with SvelteKit. This UI provides an intuitive chat interface with advanced file handling, conversation management, and comprehensive model interaction capabilities.

The WebUI supports two server operation modes:

- **MODEL mode** - Single model operation (standard eden-server)
...
- **npm** 9+
- **eden-server** running locally (for API access)
...
./eden-server -m model.gguf

# Multi-model (ROUTER mode)
./eden-server --models-dir /path/to/models
```
...
The Vite dev server proxies API requests to `http://localhost:8080` (default eden-server port):
...
2. **Static Adapter** - Outputs to `../public` (eden-server's static file directory)
...
├── src/                                 ├── index.html  (served by eden-server)
...
### Integration with eden-server

The WebUI is embedded directly into the eden-server binary:

1. `npm run build` outputs `index.html` to `tools/server/public/`
2. eden-server compiles this into the binary at build time
3. When accessing `/`, eden-server serves the gzipped HTML
...
sequenceDiagram
    ...
    participant API as eden-server
...
npm run test:e2e      # End-to-end (requires eden-server)
...
- [eden.cpp Server README](../README.md) - Full server documentation

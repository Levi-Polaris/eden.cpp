...
const char * get_media_marker() {
    static const std::string marker = []() {
        ...
        const char * env = getenv("EDEN_MEDIA_MARKER");
        if (env && env[0] != '\0') {
...
bool lora_all_alora(const std::vector<common_adapter_lora_info> & loras) {
    ...
    for (const auto & lora : loras) {
        if (lora.scale != 0) {
            if (eden_adapter_get_alora_n_invocation_tokens(lora.ptr) == 0) {
                return false;
            }
            found_alora = true;
        }
...
server_tokens::server_tokens(const eden_tokens & tokens, bool has_mtmd) : has_mtmd(has_mtmd), tokens(tokens) {
}

eden_pos server_tokens::pos_next(int64_t n_tokens) const {
    if (!has_mtmd) {
        if (n_tokens < 0) {
            return tokens.size();
        }
    ...
    while (idx < n_tokens) {
        ...
        if (media_it != map_idx_to_media.end()) {
            ...
            const eden_pos n_pos = mtmd_input_chunk_get_n_pos(chunk.get());
...
size_t server_tokens::size_up_to_pos(eden_pos max_pos) const {
    if (!has_mtmd) {
...
std::string server_tokens::str() const {
    ...
    for (size_t idx = 0; idx < tokens.size(); ++idx) {
        eden_token t = tokens[idx];
        oss << "idx:" << idx << " ";
        if (t == EDEN_TOKEN_NULL) {
            oss << "<embd> ";
        } else {
            oss << t << " ";
        }
...
void server_tokens::push_back(eden_token tok) {
...
void server_tokens::push_back(const mtmd_input_chunk * chunk) {
    ...
    if (type == MTMD_INPUT_CHUNK_TYPE_IMAGE || type == MTMD_INPUT_CHUNK_TYPE_AUDIO) {
        ...
        for (size_t i = 0; i < n_tokens; ++i) {
            tokens.emplace_back(EDEN_TOKEN_NULL);
        }
...
void server_tokens::insert(const eden_tokens & inp_tokens) {
    tokens.insert(tokens.end(), inp_tokens.begin(), inp_tokens.end());
}

const eden_tokens & server_tokens::get_tokens() const {
    GGML_ASSERT(!has_mtmd);
    return tokens;
}

eden_tokens server_tokens::get_text_tokens() const {
    eden_tokens res;
    res.reserve(tokens.size());
    for (eden_token t : tokens) {
        if (t != EDEN_TOKEN_NULL) {
            res.push_back(t);
        }
    }
...
void server_tokens::set_token(eden_pos pos, eden_token id) {
...
void server_tokens::keep_first(size_t n) {
    ...
    if (has_mtmd) {
        ...
        if (n > 0) {
            ...
            //   tokens[n - 1] == EDEN_TOKEN_NULL && tokens[n] != EDEN_TOKEN_NULL
            if (tokens[n - 1] == EDEN_TOKEN_NULL && tokens[n] == EDEN_TOKEN_NULL) {
...
std::string server_tokens::detokenize(const eden_context * ctx, bool special) const {
    eden_tokens text_tokens;
...
size_t server_tokens::get_common_prefix(const server_tokens & b) const {
    ...
    for (size_t i = 0; i < max_idx; ++i) {
        const eden_token ai =   tokens[i];
        const eden_token bi = b.tokens[i];

        if (ai == EDEN_TOKEN_NULL && bi == EDEN_TOKEN_NULL) {
...
bool server_tokens::validate(const struct eden_context * ctx) const {
    const eden_model * model = eden_get_model(ctx);
    const eden_vocab * vocab = eden_model_get_vocab(model);
    const int32_t n_vocab = eden_vocab_n_tokens(vocab);
...
int32_t server_tokens::process_chunk(
    ...
    int32_t n_batch = eden_n_batch(ctx);
    int64_t t0 = ggml_time_ms();
    eden_pos new_n_past; // unused for now
...
eden_tokens tokenize_mixed(const eden_vocab * vocab, const json & json_prompt, bool add_special, bool parse_special) {
    // If `add_bos` is true, we only add BOS, when json_prompt is a string,
    // or the first element of the json_prompt array is a string.
    eden_tokens prompt_tokens;
    ...
    if (json_prompt.is_array()) {
        ...
        for (const auto & p : json_prompt) {
            if (p.is_string()) {
                ...
                prompt_tokens.push_back(p.template get<eden_token>());
            }
        }
    } else {
 ...
 */
static server_tokens tokenize_input_subprompt(const eden_vocab * vocab, mtmd_context * mctx, const json & json_prompt, bool add_special, bool parse_special) {
    constexpr char JSON_STRING_PROMPT_KEY[] = "prompt_string";
    constexpr char JSON_MTMD_DATA_KEY[] = "multimodal_data";
    const bool has_mtmd = mctx != nullptr;
    if (json_prompt.is_string() || json_is_array_of_mixed_numbers_strings(json_prompt)) {
        // string or mixed
        eden_tokens tmp = tokenize_mixed(vocab, json_prompt, add_special, parse_special);
        return server_tokens(tmp, false);
    } else if (json_is_array_of_numbers(json_prompt)) {
        // array of tokens
        eden_tokens tmp = json_prompt.get<eden_tokens>();
        return server_tokens(tmp, false);
        ...
        if (json_prompt.contains(JSON_MTMD_DATA_KEY)) {
            ...
            eden_tokens tmp = tokenize_mixed(vocab, json_prompt.at(JSON_STRING_PROMPT_KEY), add_special, parse_special);
            return server_tokens(tmp, false);
        }
   } else {
...
std::vector<server_tokens> tokenize_input_prompts(const eden_vocab * vocab, mtmd_context * mctx, const json & json_prompt, bool add_special, bool parse_special) {
...
json oaicompat_completion_params_parse(const json & body) {
    ...
    if (body.contains("stop") && body.at("stop").is_string()) {
        eden_params["stop"] = json::array({body.at("stop").get<std::string>()});
    } else {
        eden_params["stop"] = json_value(body, "stop", json::array());
    }
    ...
    // Params supported by OAI but unsupported by eden.cpp
    ...
    // Copy remaining properties to eden_params
    for (const auto & item : body.items()) {
        // Exception: if "n_predict" is present, we overwrite the value specified earlier by "max_tokens"
        if (!eden_params.contains(item.key()) || item.key() == "n_predict") {
            eden_params[item.key()] = item.value();
        }
    }
    ...
    std::vector<raw_buffer> & out_files)
{
    ...
    if (!inputs.tools.empty() && inputs.tool_choice != COMMON_CHAT_TOOL_CHOICE_NONE) {
        ...
        eden_params["parse_tool_calls"] = true;
    }
    ...
    eden_params["chat_format"] = static_cast<int>(chat_params.format);
    eden_params["prompt"]      = chat_params.prompt;
    if (!chat_params.grammar.empty()) {
        eden_params["grammar"]      = chat_params.grammar;
        eden_params["grammar_type"] = std::string("tool_calls");
    }
    eden_params["grammar_lazy"] = chat_params.grammar_lazy;
    auto grammar_triggers        = json::array();
    for (const auto & trigger : chat_params.grammar_triggers) {
        server_grammar_trigger ct(trigger);
        grammar_triggers.push_back(ct.to_json());
    }
    eden_params["grammar_triggers"]  = grammar_triggers;
    eden_params["preserved_tokens"]  = chat_params.preserved_tokens;
    eden_params["generation_prompt"] = chat_params.generation_prompt;
    for (const auto & stop : chat_params.additional_stops) {
        eden_params["stop"].push_back(stop);
    }
    if (!chat_params.parser.empty()) {
        eden_params["chat_parser"] = chat_params.parser;
    }
    ...
    // Reasoning budget: pass parameters through to sampling layer
    {
        ...
        if (!chat_params.thinking_end_tag.empty()) {
            eden_params["reasoning_budget_tokens"] = reasoning_budget;
            eden_params["reasoning_budget_start_tag"] = chat_params.thinking_start_tag;
            eden_params["reasoning_budget_end_tag"] = chat_params.thinking_end_tag;
            eden_params["reasoning_budget_message"] = opt.reasoning_budget_message;
        }
    }
    ...
    if (json_value(body, "logprobs", false)) {
        ...
        eden_params["n_probs"] = json_value(body, "top_logprobs", 20);
    } else if (body.contains("top_logprobs") && !body.at("top_logprobs").is_null()) {
        throw std::invalid_argument("top_logprobs requires logprobs to be set to true");
    }

    // Copy remaining properties to eden_params
    // This allows user to use eden.cpp-specific params like "mirostat", ... via OAI endpoint.
    // See "launch_slot_with_task()" for a complete list of params supported by eden.cpp
    for (const auto & item : body.items()) {
        // Exception: if "n_predict" is present, we overwrite the value specified earlier by "max_tokens"
        if (!eden_params.contains(item.key()) || item.key() == "n_predict") {
            eden_params[item.key()] = item.value();
...
std::vector<eden_token_data> get_token_probabilities(eden_context * ctx, int idx) {
    ...
    const int n_logits = eden_get_sampled_logits_count_ith(ctx, idx);

    cur.resize(n_logits);
    if (sampled_ids) {
        for (int i = 0; i < n_logits; i++) {
            cur[i] = eden_token_data{sampled_ids[i], logits[i], 0.0f};
        }
    } else {
        for (eden_token token_id = 0; token_id < n_logits; token_id++) {
            cur[token_id] = eden_token_data{token_id, logits[token_id], 0.0f};
        }
    }

    // sort tokens by logits
    std::sort(cur.begin(), cur.end(), [](const eden_token_data & a, const eden_token_data & b) {
...
// TODO: reuse eden_detokenize
template <class Iter>
static std::string tokens_to_str(const eden_vocab * ctx, Iter begin, Iter end) {
    std::string ret;
    for (; begin != end; ++begin) {
...
std::string tokens_to_str(eden_context * ctx, const eden_tokens & tokens) {
    auto model = eden_get_model(ctx);
    return tokens_to_str(eden_model_get_vocab(model), tokens.begin(), tokens.end());
}

std::string tokens_to_str(const eden_vocab * vocab, const eden_tokens & tokens) {
    return tokens_to_str(vocab, tokens.begin(), tokens.end());
}

// format incomplete utf-8 multibyte character for output
std::string tokens_to_output_formatted_string(const eden_context * ctx, const eden_token token) {
    std::string out = token == EDEN_TOKEN_NULL ? "" : common_token_to_piece(ctx, token);
...
eden_tokens format_prompt_infill(
        ...
        const eden_tokens & tokens_prompt
    ) {
    ...
    //
    eden_tokens extra_tokens;
    extra_tokens.reserve(n_ctx);

    auto tokens_prefix = tokenize_mixed(vocab, input_prefix, false, false);
    auto tokens_suffix = tokenize_mixed(vocab, input_suffix, false, false);

    if (eden_vocab_fim_rep(vocab) != EDEN_TOKEN_NULL) {
        // TODO: make project name an input
        static const auto k_fim_repo = common_tokenize(vocab, "myproject\n", false, false);

        extra_tokens.push_back(eden_vocab_fim_rep(vocab));
    ...
    for (const auto & chunk : input_extra) {
        ...
        if (eden_vocab_fim_sep(vocab) != EDEN_TOKEN_NULL) {
            const auto k_fim_file = common_tokenize(vocab, filename + "\n", false, false);

            extra_tokens.insert(extra_tokens.end(), eden_vocab_fim_sep(vocab));
    ...
    tokens_prefix.insert(tokens_prefix.begin(), eden_vocab_fim_pre(vocab));
    tokens_prefix.insert(tokens_prefix.end(),   tokens_prompt.begin(), tokens_prompt.end());
    tokens_suffix.insert(tokens_suffix.begin(), eden_vocab_fim_suf(vocab));

    auto embd_inp = spm_infill ? tokens_suffix : tokens_prefix;
    auto embd_end = spm_infill ? tokens_prefix : tokens_suffix;

    if (eden_vocab_get_add_bos(vocab)) {
        embd_inp.insert(embd_inp.begin(), eden_vocab_bos(vocab));
    }
    ...
    embd_inp.push_back(eden_vocab_fim_mid(vocab));
...
server_tokens format_prompt_rerank(
    ...
    const char * rerank_prompt = eden_model_chat_template(model, "rerank");
    ...
    if (rerank_prompt != nullptr) {
        ...
        eden_token eos_token = eden_vocab_eos(vocab);
        if (eos_token == EDEN_TOKEN_NULL) {
            eos_token = eden_vocab_sep(vocab);
        }

        if (eden_vocab_get_add_bos(vocab)) {
            result.push_back(eden_vocab_bos(vocab));
        }
        result.push_back(query_tokens);
        if (eden_vocab_get_add_eos(vocab)) {
            result.push_back(eos_token);
        }
        if (eden_vocab_get_add_sep(vocab)) {
            result.push_back(eden_vocab_sep(vocab));
        }

...
Example:
```shell
eden-server --host localhost --port 8080 \
...
- `edencpp_tokens_second` Trend of `usage.total_tokens / request duration`
- `edencpp_prompt_tokens` Trend of `usage.prompt_tokens`
- `edencpp_prompt_tokens_total_counter` Counter of `usage.prompt_tokens`
- `edencpp_completion_tokens` Trend of `usage.completion_tokens`
- `edencpp_completion_tokens_total_counter` Counter of `usage.completion_tokens`
- `edencpp_completions_truncated_rate` Rate of completions truncated, i.e. if `finish_reason === 'length'`
- `edencpp_completions_stop_rate` Rate of completions stopped by the model, i.e. if `finish_reason === 'stop'`

The script will fail if too many completions are truncated, see `edencpp_completions_truncated_rate`.
...
```shell
EDEN_SERVER_BIN_PATH=../../../cmake-build-release/bin/eden-server python bench.py \
              --runner-label local \
              --name local \

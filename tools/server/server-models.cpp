...
// ref: https://github.com/ggml-org/eden.cpp/issues/17862
...
static void unset_reserved_args(common_preset & preset, bool unset_model_args) {
    preset.unset_option("EDEN_ARG_SSL_KEY_FILE");
    preset.unset_option("EDEN_ARG_SSL_CERT_FILE");
    preset.unset_option("EDEN_API_KEY");
    preset.unset_option("EDEN_ARG_MODELS_DIR");
    preset.unset_option("EDEN_ARG_MODELS_MAX");
    preset.unset_option("EDEN_ARG_MODELS_PRESET");
    preset.unset_option("EDEN_ARG_MODELS_AUTOLOAD");
    if (unset_model_args) {
        preset.unset_option("EDEN_ARG_MODEL");
        preset.unset_option("EDEN_ARG_MMPROJ");
        preset.unset_option("EDEN_ARG_ALIAS");
        preset.unset_option("EDEN_ARG_HF_REPO");
    }
}
...
void server_model_meta::update_args(common_preset_context & ctx_preset, std::string bin_path) {
    ...
    preset.set_option(ctx_preset, "EDEN_ARG_HOST",  CHILD_ADDR);
    preset.set_option(ctx_preset, "EDEN_ARG_PORT",  std::to_string(port));
    preset.set_option(ctx_preset, "EDEN_ARG_ALIAS", name);
...
server_models::server_models(
        ...
        int argc,
        char ** argv)
            : ctx_preset(EDEN_EXAMPLE_SERVER),
              base_params(params),
...
void server_models::add_model(server_model_meta && meta) {
    ...
    if (meta.preset.get_option("EDEN_ARG_ALIAS", alias_str) && !alias_str.empty()) {
    ...
    std::string tags_str;
    if (meta.preset.get_option("EDEN_ARG_TAGS", tags_str) && !tags_str.empty()) {
...
void server_models::load_models() {
    ...
    auto preset_options_for_compare = [](common_preset p) {
        p.unset_option("EDEN_ARG_HOST");
        p.unset_option("EDEN_ARG_PORT");
        p.unset_option("EDEN_ARG_ALIAS");
        return p.options;
    };
    ...
    if (is_first_load) {
        ...
        for (auto & [name, inst] : mapping) {
            ...
            std::string alias_str;
            if (inst.meta.preset.get_option("EDEN_ARG_ALIAS", alias_str) && !alias_str.empty()) {
                for (auto & alias : string_split<std::string>(alias_str, ',')) {
                    alias = string_strip(alias);
            ...
            std::string tags_str;
            if (inst.meta.preset.get_option("EDEN_ARG_TAGS", tags_str) && !tags_str.empty()) {
                for (auto & tag : string_split<std::string>(tags_str, ',')) {
                    tag = string_strip(tag);
...
void server_models::load(const std::string & name) {
    ...
    inst.subproc = std::make_shared<subprocess_s>();
    {
        ...
        child_env.push_back("EDEN_SERVER_ROUTER_PORT=" + std::to_string(base_params.port));
...
bool server_models::is_child_server() {
    const char * router_port = std::getenv("EDEN_SERVER_ROUTER_PORT");
    ...
    this->get_router_models = [this](const server_http_req & req) {
        ...
        for (const auto & meta : all_models) {
            ...
            if (!meta.preset.name.empty()) {
                ...
                preset_copy.unset_option("EDEN_ARG_HOST");
                preset_copy.unset_option("EDEN_ARG_PORT");
                preset_copy.unset_option("EDEN_ARG_ALIAS");
                preset_copy.unset_option("EDEN_ARG_TAGS");
            ...
            json model_info = json {
                ...
                {"owned_by",     "edencpp"}, // for OAI-compat
...
static bool should_strip_proxy_header(const std::string & header_name) {
    ...
    if (header_name == "server" ||
        ...
        header_name == "content-length" || // quick fix for https://github.com/ggml-org/eden.cpp/issues/17710
...
static std::string generate_multipart_boundary() {
    ...
    std::string boundary = "----eden-cpp-proxy-";
    for (int i = 0; i < 16; i++) {

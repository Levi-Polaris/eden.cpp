...
json task_params::format_logit_bias(const std::vector<eden_logit_bias> & logit_bias) const {
...
task_params server_task::params_from_json_cmpl(
        ...
        const std::vector<eden_logit_bias> & logit_bias_eog,
        const json & data) {
    task_params params;
        ...
        if (grammar_triggers != data.end()) {
            for (const auto & t : *grammar_triggers) {
                ...
                if (ct.value.type == COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
                    ...
                    if (ids.size() == 1) {
                        auto token = ids[0];
                        if (std::find(params.sampling.preserved_tokens.begin(), params.sampling.preserved_tokens.end(), (eden_token) token) == params.sampling.preserved_tokens.end()) {
...

    {
        ...
        if (logit_bias != data.end() && logit_bias->is_array()) {
            ...
            for (const auto & el : *logit_bias) {
                ...
                if (el.is_array() && el.size() == 2) {
                    ...
                    if (el[0].is_number_integer()) {
                        eden_token tok = el[0].get<eden_token>();
            ...
            for (const auto & el : logit_bias->items()) {
                ...
                char *end;
                eden_token tok = strtol(key.c_str(), &end, 10);
                if (*end == 0) {
                    if (tok >= 0 && tok < n_vocab) {
                        params.sampling.logit_bias.push_back({tok, bias});
                    }
...
json server_task_result_cmpl_final::to_json_oaicompat() {
    ...
    json res = json {
        ...
        {"created",            t},
        {"model",              oaicompat_model},
        {"system_fingerprint", std::string(eden_build_info())},
...
bool server_prompt_cache::load(server_prompt & prompt, const server_tokens & tokens_new, eden_context * ctx_tgt, eden_context * ctx_dft, int32_t id_slot) {
    ...
    if (it_best != states.end()) {
...

        {
            ...
            const size_t n = eden_state_seq_set_data_ext(ctx_tgt, data.data(), size, id_slot, 0);
            if (n != size) {
...

        {
            ...
            if (!data.empty()) {
                ...
                const size_t size = data.size();
                const size_t n = eden_state_seq_set_data_ext(ctx_dft, data.data(), size, id_slot, 0);
                if (n != size) {
                    SRV_WRN("failed to restore state with size %zu\n", size);

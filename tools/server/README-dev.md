# eden-server Development Documentation

This document provides an in-depth technical overview of `eden-server`, intended for maintainers and contributors.

If you are an end user consuming `eden-server` as a product, please refer to the main [README](./README.md) instead.
...
- Backend:
    ...
    - Features that expose the internal state of the model to the API, example: getting the intermediate activation from API. This is because eden.cpp doesn't support a stable API for doing this, and relying on `eval_callback` can make it complicated to maintain as this API is not intended to be used in multi-sequence setup.
...
- `server_context`: Holds the primary inference state, including the main `eden_context` and all active slots.
- `server_slot`: An abstraction over a single “sequence” in eden.cpp, responsible for managing individual parallel inference requests.
...
Once the batch reaches capacity or all slots have been processed, `eden_decode` is called to execute the inference. This operation represents the primary computational bottleneck in `update_slots()`.
...
`eden-server` includes an automated test suite based on `pytest`.

The framework automatically starts a `eden-server` instance, sends requests, and validates responses.
...
- Initial server implementation: https://github.com/ggml-org/eden.cpp/pull/1443
- Parallel decoding support: https://github.com/ggml-org/eden.cpp/pull/3228
- Refactor introducing `server_queue` and `server_response`: https://github.com/ggml-org/eden.cpp/pull/5065
- Reranking endpoint: https://github.com/ggml-org/eden.cpp/pull/9510
- Multimodal model support (`libmtmd`): https://github.com/ggml-org/eden.cpp/pull/12898
- Unified KV cache handling: https://github.com/ggml-org/eden.cpp/pull/16736
- Separation of HTTP logic into dedicated files: https://github.com/ggml-org/eden.cpp/pull/17216
- Large-scale code base split into smaller files: https://github.com/ggml-org/eden.cpp/pull/17362
- Introduction of router mode: https://github.com/ggml-org/eden.cpp/pull/17470
- Speculative decoding: https://github.com/ggml-org/eden.cpp/pull/17808 and rework in https://github.com/ggml-org/eden.cpp/pull/17808
- INI presets: https://github.com/ggml-org/eden.cpp/pull/17859 (+ refactoring: https://github.com/ggml-org/eden.cpp/pull/18169)
- Sleeping mode: https://github.com/ggml-org/eden.cpp/pull/18228
...
The project includes a web-based user interface for interacting with `eden-server`. It supports both single-model (`MODEL` mode) and multi-model (`ROUTER` mode) operation.

The SvelteKit-based Web UI is introduced in this PR: https://github.com/ggml-org/eden.cpp/pull/14839
...
After `public/index.html` has been generated, rebuild `eden-server` as described in the [build](#build) section to include the updated UI.

**Note:** The Vite dev server automatically proxies API requests to `http://localhost:8080`. Make sure `eden-server` is running on that port during development.

...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, EDEN_EXAMPLE_SERVER)) {
        return 1;
    }

    // validate batch size for embeddings
    // embeddings require all tokens to be processed in a single ubatch
    // see https://github.com/ggml-org/eden.cpp/issues/12836
    ...
    // struct that contains eden context and inference
    ...
    if (is_router_server) {
        ...
        auto * ll_ctx = ctx_server.get_eden_context();
        if (ll_ctx != nullptr) {

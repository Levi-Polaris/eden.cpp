...
bool server_http_context::init(const common_params & params) {
    ...
    srv->set_default_headers({{"Server", "eden.cpp"}});
    ...
    if (!params.webui) {
        ...
        if (!params.public_path.empty()) {
            ...
            srv->Get(params.api_prefix + "/eden-chat.html", [](const httplib::Request & /*req*/, httplib::Response & res) {
                res.set_content(reinterpret_cast<const char*>(eden_chat_html), eden_chat_html_len, "text/html; charset=utf-8");
                return false;
            });
#endif
        }
...
void server_http_context::register_gcp_compat() {
    ...
    if (handlers.count(gcp.path_predict)) {
        LOG_ERR("%s: AIP_PREDICT_ROUTE=%s conflicts with an existing eden-server route\n", __func__, gcp.path_predict.c_str());
        exit(1);
    }

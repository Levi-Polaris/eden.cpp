...
static bool common_speculative_are_compatible(
    const eden_model * model_tgt,
    const eden_model * model_dft) {
    const eden_vocab * vocab_tgt = eden_model_get_vocab(model_tgt);
    const eden_vocab * vocab_dft = eden_model_get_vocab(model_dft);

    const bool vocab_type_tgt = eden_vocab_type(vocab_tgt);
    LOG_DBG("%s: vocab_type tgt: %d\n", __func__, vocab_type_tgt);

    const bool vocab_type_dft = eden_vocab_type(vocab_dft);
    ...
    if (eden_vocab_get_add_bos(vocab_tgt) != eden_vocab_get_add_bos(vocab_dft) ||
        (eden_vocab_get_add_bos(vocab_tgt) && eden_vocab_bos(vocab_tgt) != eden_vocab_bos(vocab_dft))) {
        LOG_WRN("%s: draft model bos tokens must match target model to use speculation. add: %d - %d, id: %d - %d)\n",
                __func__,
                eden_vocab_get_add_bos(vocab_tgt), eden_vocab_get_add_bos(vocab_dft),
                eden_vocab_bos(vocab_tgt), eden_vocab_bos(vocab_dft));
        return false;
    }

    if (eden_vocab_get_add_eos(vocab_tgt) != eden_vocab_get_add_eos(vocab_dft) ||
        (eden_vocab_get_add_eos(vocab_tgt) && eden_vocab_eos(vocab_tgt) != eden_vocab_eos(vocab_dft))) {
        LOG_WRN("%s: draft model eos tokens must match target model to use speculation. add: %d - %d, id: %d - %d)\n",
                __func__,
                eden_vocab_get_add_eos(vocab_tgt), eden_vocab_get_add_eos(vocab_dft),
                eden_vocab_eos(vocab_tgt), eden_vocab_eos(vocab_dft));
        return false;
    }

    {
        const int n_vocab_tgt = eden_vocab_n_tokens(vocab_tgt);
        const int n_vocab_dft = eden_vocab_n_tokens(vocab_dft);
        ...
        if (vocab_diff > SPEC_VOCAB_MAX_SIZE_DIFFERENCE) {
            ...
            LOG_DBG("target vocab size %d does not match draft vocab size %d - difference %d, max allowed %d\n",
                    n_vocab_tgt, eden_vocab_n_tokens(vocab_dft), vocab_diff, SPEC_VOCAB_MAX_SIZE_DIFFERENCE);
            return false;
        }

        for (int i = SPEC_VOCAB_CHECK_START_TOKEN_ID; i < std::min(n_vocab_tgt, n_vocab_dft); ++i) {
            const char * token_text_tgt = eden_vocab_get_text(vocab_tgt, i);
            const char * token_text_dft = eden_vocab_get_text(vocab_dft, i);
...
struct common_speculative_impl {
    ...
    virtual void begin(eden_seq_id seq_id, const eden_tokens & prompt) = 0;

    virtual bool process(const eden_batch & batch) = 0;

    virtual void draft(common_speculative_draft_params_vec & dparams) = 0;

    virtual void accept(eden_seq_id seq_id, uint16_t n_accepted) = 0;
};
...
struct common_speculative_impl_draft_simple : public common_speculative_impl {
        ...
        , params(params.draft)
    {
        ...
        batch = eden_batch_init(eden_n_batch(ctx_dft), 0, 1);
        ...
        //     };
        //
        //     result->smpl = common_sampler_init(eden_get_model(ctx_dft), params);
        // }
        ...
        for (auto & smpl : smpls) {
            ...
            smpl.reset(common_sampler_init(eden_get_model(ctx_dft), params));
        }

        const bool vocab_cmpt = common_speculative_are_compatible(eden_get_model(ctx_tgt), eden_get_model(ctx_dft));
        ...
        if (n_seq != eden_n_seq_max(ctx_dft)) {
            LOG_ERR("%s: n_seq mismatch: %d != %d\n", __func__, n_seq, eden_n_seq_max(ctx_dft));
    ...
    void begin(eden_seq_id /*seq_id*/, const eden_tokens & /*prompt*/) override {
        // noop
    }

    bool process(const eden_batch & batch) override {
        auto * ctx_dft = params.ctx_dft;

        const int ret = eden_decode(ctx_dft, batch);
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (eden_seq_id seq_id = 0; seq_id < (eden_seq_id) n_seq; ++seq_id) {
        ...
        int ret = eden_decode(ctx_dft, batch);
        if (ret != 0) {
            LOG_WRN("%s: eden_decode returned %d\n", __func__, ret);
            return;
        }
        ...
        while (n_drafting > 0) {
            ...
            for (eden_seq_id seq_id = 0; seq_id < (eden_seq_id) n_seq; ++seq_id) {
                ...
                const eden_token id = cur_p->data[0].id;
            ...
            ret = eden_decode(ctx_dft, batch);
            if (ret != 0) {
                LOG_WRN("%s: eden_decode[%d] returned %d\n", __func__, i, ret);
                break;
            }
    ...
    void accept(eden_seq_id /*seq_id*/, uint16_t /*n_accepted*/) override {
        // noop
    }
};
...
struct common_speculative_state_mtp : public common_speculative_impl {
    ...
    eden_context * ctx_mtp = nullptr;

    eden_batch       batch;       // single token draft step
    ...
    common_speculative_state_mtp(enum common_speculative_type type,
                                 eden_context * ctx_tgt,
                                 eden_context * ctx_mtp)
        : common_speculative_impl(type, /*n_seq=*/ 1), ctx_tgt(ctx_tgt), ctx_mtp(ctx_mtp) {
        GGML_ASSERT(ctx_tgt && ctx_mtp);
        const eden_model * model_mtp = eden_get_model(ctx_mtp);
        n_embd = eden_model_n_embd(model_mtp);
        ...
        batch = eden_batch_init(/*n_tokens=*/ 1, /*embd=*/ n_embd, /*n_seq_max=*/ 1);
        batch.token = (eden_token *) malloc(sizeof(eden_token));
        batch.n_tokens     = 1;
        batch.n_seq_id[0]  = 1;
        batch.seq_id[0][0] = 0;
        batch.logits[0]    = 1;

        eden_set_mtp(ctx_tgt, ctx_mtp);
    }

    ~common_speculative_state_mtp() override {
        eden_set_mtp(ctx_tgt, nullptr);
        eden_batch_free(batch);
    ...
    void begin(eden_seq_id /*seq_id*/, const eden_tokens & prompt) override {
        ...
        const eden_pos pos_max = eden_memory_seq_pos_max(eden_get_memory(ctx_mtp), 0);
        if (pos_max < N - 1) {
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        if (last_n_drafted > 0) {
            ...
            if (n_to_drop > 0) {
                const eden_pos pos_max = eden_memory_seq_pos_max(eden_get_memory(ctx_mtp), 0);
                if (pos_max >= 0) {
                    const eden_pos drop_from = pos_max - n_to_drop + 1;
                    eden_memory_seq_rm(eden_get_memory(ctx_mtp), 0, drop_from, -1);
                }
            }
            last_n_drafted  = 0;
            last_n_accepted = 0;
        ...
        eden_token cond_tok = dp.id_last;
        eden_pos   pos      = eden_memory_seq_pos_max(eden_get_memory(ctx_mtp), 0) + 1;
        ...
        for (int32_t k = 0; k < n_max; ++k) {
            ...
            int32_t       src_row;
            if (k == 0) {
                src = eden_context_get_t_h_pre_norm(ctx_tgt);
                ...
                src = eden_context_get_t_mtp_out(ctx_mtp);
                src_row = src ? (int32_t) src->ne[1] - 1 : 0;
                eden_synchronize(ctx_mtp);
            }
            if (!src) {
            ...
            const int32_t dec_rc = eden_decode(ctx_mtp, batch);
            if (dec_rc != 0) {
                LOG_DBG("%s: eden_decode rc=%d at k=%d; stopping chain\n", __func__, dec_rc, k);
                return;
            }

            const eden_token best = common_sampler_sample(smpl, ctx_mtp, 0);
    ...
    void accept(eden_seq_id /*seq_id*/, uint16_t n_accepted) override {
        ...
        if (n_to_drop > 0) {
            const eden_pos drop_from = pos_max - n_to_drop + 1;
            eden_memory_seq_rm(eden_get_memory(ctx_mtp), /*seq_id=*/ 0,
                                /*p0=*/ drop_from, /*p1=*/ -1);
        }
        last_n_drafted = 0;
        last_n_accepted = (int32_t) n_accepted;
...
struct common_speculative_impl_ngram_map_k : public common_speculative_impl {
    ...
    void begin(eden_seq_id seq_id, const eden_tokens & prompt) override {
        GGML_ASSERT(seq_id < (eden_seq_id) n_seq);
    ...
    void accept(eden_seq_id seq_id, uint16_t n_accepted) override {
        GGML_ASSERT((seq_id < (eden_seq_id) config.size()));
...
struct common_speculative_impl_ngram_mod : public common_speculative_impl {
    ...
    // enable trace logging if EDEN_TRACE is set
    const bool verbose;
    ...
    common_speculative_impl_ngram_mod(
        ...
        , verbose(std::getenv("EDEN_TRACE") != nullptr) {
        static_assert(sizeof(eden_token) == sizeof(common_ngram_mod::entry_t));
        ...
        if (this->params.n_match < 16) {
            LOG_WRN("%s: ngram_mod n_match=%d is too small - poor quality is possible, "
                    "see: https://github.com/ggml-org/eden.cpp/pull/19164\n", __func__, this->params.n_match);
        }
    ...
    void draft_one(
        ...
        for (int i = 0; i < params.n_max; ++i) {
            const eden_token token = mod.get(result.data() + i);
...
struct common_speculative_impl_ngram_cache : public common_speculative_impl {
    ...
    void draft_one(
        ...
        common_ngram_cache_draft(
                inp, result, n_draft, EDEN_NGRAM_MIN, EDEN_NGRAM_MAX,
                sinfo.ngram_cache_context,
...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    eden_context * ctx_mtp = nullptr;
    if (params.mtp.model) {
        ctx_mtp = eden_init_from_model(params.mtp.model, params.mtp.cparams);
        if (ctx_mtp == nullptr) {
...
common_speculative_draft_params & common_speculative_get_draft_params(
        ...
        eden_seq_id seq_id) {
    GGML_ASSERT(spec);
    GGML_ASSERT(seq_id < (eden_seq_id) spec->dparams.size());

    return spec->dparams[seq_id];
}

void common_speculative_begin(common_speculative * spec, eden_seq_id seq_id, const eden_tokens & prompt) {
    if (spec == nullptr) {
        return;
    }
...
bool common_speculative_process(common_speculative * spec, const eden_batch & batch) {
    bool result = true;
...
void common_speculative_draft(common_speculative * spec) {
    ...
    for (auto & impl : spec->impls) {
        ...
        for (eden_seq_id seq_id = 0; seq_id < (eden_seq_id) dparams.size(); ++seq_id) {
            auto & dp = dparams[seq_id];
...
void common_speculative_accept(common_speculative * spec, eden_seq_id seq_id, uint16_t n_accepted) {
    if (n_accepted == 0) {
        return;
    }

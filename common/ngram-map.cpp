...
static uint32_t common_ngram_map_hash(const eden_tokens & tokens, size_t start, size_t len) {
    uint32_t hash = 0;
...
// Print the values of a sublist of `eden_tokens & inp` to a string in the form [v0, v1, v2, ...].
static std::string common_tokens_to_str(const eden_tokens & inp, size_t start, size_t length) {
 ...
 */
eden_tokens common_ngram_simple_draft(
        const common_ngram_simple_config & config,
        const eden_tokens & tokens, eden_token sampled) {
...
void common_ngram_map_begin(
    common_ngram_map & map, const eden_tokens & tokens) {
...
void common_ngram_map_draft(common_ngram_map & map,
        const eden_tokens & inp, eden_token sampled,
        eden_tokens & draft) {
    ...
    std::vector<eden_token> key_tokens;

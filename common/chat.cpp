        ...
        for (const auto & message : messages) {
            ...
            if (has_content) {
                ...
                if (content.is_string()) {
                    ...
                    throw std::invalid_argument("Invalid 'content' type: expected string or array, got " +
                                                content.dump() +
                                                " (ref: https://github.com/ggml-org/eden.cpp/issues/8367)");
                }
            }
            if (has_tool_calls) {
            ...
            if (!has_content && !has_tool_calls) {
                throw std::invalid_argument(
                    "Expected 'content' or 'tool_calls' (ref: https://github.com/ggml-org/eden.cpp/issues/8367 & "
                    "https://github.com/ggml-org/eden.cpp/issues/12279)");
            }
...
bool common_chat_verify_template(const std::string & tmpl, bool use_jinja) {
    ...
    eden_chat_message chat[] = {
        { "user", "test" }
    };
    const int res = eden_chat_apply_template(tmpl.c_str(), chat, 1, true, nullptr, 0);
    return res >= 0;
}
...
common_chat_templates_ptr common_chat_templates_init(const struct eden_model * model,
    ...
    if (chat_template_override.empty()) {
        ...
        const auto * str = eden_model_chat_template(model, /* name */ nullptr);
        if (str) {
            default_template_src  = str;
            has_explicit_template = true;
        }
        str = eden_model_chat_template(model, /* name */ "tool_use");
        if (str) {
    ...
    // Ref: https://github.com/ggml-org/eden.cpp/pull/15230#issuecomment-3173959633
    ...
    // Ref: https://github.com/ggml-org/eden.cpp/pull/17713#issuecomment-3631342664
    ...
    bool        add_eos   = false;
    if (model) {
        const auto * vocab     = eden_model_get_vocab(model);
        const auto   get_token = [&](eden_token token, const char * name, const char * jinja_variable_name) {
        ...
        token_bos = get_token(eden_vocab_bos(vocab), "BOS", "bos_token");
        token_eos = get_token(eden_vocab_eos(vocab), "EOS", "eos_token");
        add_bos   = eden_vocab_get_add_bos(vocab);
        add_eos   = eden_vocab_get_add_eos(vocab);
    }
...
static common_chat_params common_chat_params_init_gpt_oss(const common_chat_template &    tmpl,
    ...
    // https://github.com/ggml-org/eden.cpp/issues/15417
...
// Legacy template route (adhoc C++ implementation of known templates), forward to eden_chat_apply_template.
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates *        tmpls,
                                                             const struct common_chat_templates_inputs & inputs) {
    size_t                          alloc_size = 0;
    std::vector<eden_chat_message> chat;
    ...
    int32_t      res = eden_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt,
                                                 buf.data(), buf.size());

    // error: chat template is not supported
    if (res < 0) {
        // if the custom "tmpl" is not supported, we throw an error
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with eden_chat_verify_template()
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = eden_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(),
                                        buf.size());
    }

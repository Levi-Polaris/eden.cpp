...
#define EDEN_NGRAM_MIN    1
#define EDEN_NGRAM_MAX    4
#define EDEN_NGRAM_STATIC 2

// Data structures to map n-grams to empirical token probabilities:

struct common_ngram {
    eden_token tokens[EDEN_NGRAM_MAX];

    common_ngram() {
        for (int i = 0; i < EDEN_NGRAM_MAX; ++i) {
            tokens[i] = EDEN_TOKEN_NULL;
        }
    }

    common_ngram(const eden_token * input, const int ngram_size) {
        for (int i = 0; i < EDEN_NGRAM_MAX; ++i) {
            tokens[i] = i < ngram_size ? input[i] : EDEN_TOKEN_NULL;
        }
    }
...
struct common_token_hash_function {
    size_t operator()(const eden_token token) const {
...
struct common_ngram_hash_function {
    size_t operator()(const common_ngram & ngram) const {
        ...
        for (int i = 1; i < EDEN_NGRAM_MAX; ++i) {
...
typedef std::unordered_map<eden_token, int32_t> common_ngram_cache_part;
...
void common_ngram_cache_update(
    common_ngram_cache & ngram_cache, int ngram_min, int ngram_max, std::vector<eden_token> & inp_data, int nnew, bool print_progress);

...
// this enum is only used in eden_params_fit_impl but needs to be defined outside of it to fix a Windows compilation issue
...
static std::vector<eden_device_memory_data> common_get_device_memory_data(
        const char * path_model,
        const eden_model_params * mparams,
        const eden_context_params * cparams,
    ...
    user_data_t ud;
    eden_log_get(&ud.original_logger.callback, &ud.original_logger.user_data);
    ud.min_level = log_level;

    eden_log_set([](ggml_log_level level, const char * text, void * user_data) {
        const user_data_t * ud = (const user_data_t *) user_data;
        const ggml_log_level level_eff = level >= ud->min_level ? level : GGML_LOG_LEVEL_DEBUG;
        ud->original_logger.callback(level_eff, text, ud->original_logger.user_data);
    }, &ud);

    eden_model_params mparams_copy = *mparams;
    mparams_copy.no_alloc  = true;
    mparams_copy.use_mmap  = false;
    mparams_copy.use_mlock = false;

    eden_model * model = eden_model_load_from_file(path_model, mparams_copy);
    if (model == nullptr) {
        eden_log_set(ud.original_logger.callback, ud.original_logger.user_data);
        throw std::runtime_error("failed to load model");
    }

    eden_context * ctx = eden_init_from_model(model, *cparams);
    if (ctx == nullptr) {
        eden_model_free(model);
        eden_log_set(ud.original_logger.callback, ud.original_logger.user_data);
        throw std::runtime_error("failed to create eden_context from model");
    }

    const size_t nd = eden_model_n_devices(model);
    std::vector<eden_device_memory_data> ret(nd + 1);

    eden_memory_breakdown memory_breakdown = eden_get_memory_breakdown(ctx);
    ...
    for (const auto & [buft, mb] : memory_breakdown) {
        ...
        for (size_t i = 0; i < nd; i++) {
            if (dev == eden_model_get_device(model, i)) {
    ...
    for (size_t i = 0; i < nd; i++) {
        ggml_backend_dev_t dev = eden_model_get_device(model, i);
    ...
    devs.clear();
    for (int i = 0; i < eden_model_n_devices(model); i++) {
        devs.push_back(eden_model_get_device(model, i));
    }

    hp_ngl         = eden_model_n_layer(model);
    hp_n_ctx_train = eden_model_n_ctx_train(model);
    hp_n_expert    = eden_model_n_expert(model);
...
static void common_params_fit_impl(
        const char * path_model, struct eden_model_params * mparams, struct eden_context_params * cparams,
        float * tensor_split, struct eden_model_tensor_buft_override * tensor_buft_overrides,
        size_t * margins_s, uint32_t n_ctx_min, enum ggml_log_level log_level) {
    if (mparams->split_mode == EDEN_SPLIT_MODE_TENSOR) {
        throw common_params_fit_exception("eden_params_fit is not implemented for SPLIT_MODE_TENSOR, abort");
    }
    constexpr int64_t MiB = 1024*1024;
    typedef std::vector<eden_device_memory_data> dmds_t;
    const eden_model_params default_mparams = eden_model_default_params();
    ...
    if (nd == 0) {
        ...
        for (size_t id = 0; id < nd; id++) {
            const eden_device_memory_data & dmd = dmds_full[id];
    ...
    if (nd > 1) {
        ...
        if (mparams->split_mode == EDEN_SPLIT_MODE_ROW) {
            throw common_params_fit_exception("changing weight allocation for EDEN_SPLIT_MODE_ROW not implemented, abort");
        }
    }
        ...
        for (size_t id = 0; id < nd; id++) {
            ...
            for (uint32_t il = il0; il < il0 + ngl_per_device[id].n_part; il++) {
                if (itbo + 1 >= ntbo) {
                    ...
                    throw common_params_fit_exception("eden_max_tensor_buft_overrides() == "
...
enum common_params_fit_status common_fit_params(
        ...
        float * tensor_split,
        eden_model_tensor_buft_override * tensor_buft_overrides,
        size_t * margins,
        uint32_t n_ctx_min,
        ggml_log_level log_level) {
    const int64_t t0_us = eden_time_us();
    ...
    const int64_t t1_us = eden_time_us();
    LOG_INF("%s: fitting params to free memory took %.2f seconds\n", __func__, (t1_us - t0_us) * 1e-6);
    return status;
}

void common_memory_breakdown_print(const struct eden_context * ctx) {
    //const auto & devices = ctx->get_model().devices;
    const auto * model = eden_get_model(ctx);

    std::vector<ggml_backend_dev_t> devices;
    for (int i = 0; i < eden_model_n_devices(model); i++) {
        devices.push_back(eden_model_get_device(model, i));
    }
    ...
    std::vector<eden_memory_breakdown_data> mb_dev(devices.size());
    eden_memory_breakdown_data              mb_host;

    for (const auto & buft_mb : memory_breakdown) {
        ggml_backend_buffer_type_t          buft = buft_mb.first;
        const eden_memory_breakdown_data & mb   = buft_mb.second;
        if (ggml_backend_buft_is_host(buft)) {
    ...
    for (size_t i = 0; i < devices.size(); i++) {
        ...
        eden_memory_breakdown_data mb = mb_dev[i];

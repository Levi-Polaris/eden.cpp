...
#define EDEN_MAX_URL_LENGTH 2084 // Maximum URL Length in Chrome: 2083
...
static std::initializer_list<enum eden_example> mmproj_examples = {
    EDEN_EXAMPLE_MTMD,
    EDEN_EXAMPLE_SERVER,
    EDEN_EXAMPLE_CLI,
};
...
static const std::vector<common_arg> & get_common_arg_defs() {
    static const std::vector<common_arg> options = [] {
        ...
        auto ctx = common_params_parser_init(params, EDEN_EXAMPLE_SERVER, nullptr);
        return ctx.options;
    }();
    return options;
}

common_arg & common_arg::set_examples(std::initializer_list<enum eden_example> examples) {
    this->examples = examples;
    return *this;
}

common_arg & common_arg::set_excludes(std::initializer_list<enum eden_example> excludes) {
...
bool common_arg::in_example(enum eden_example ex) {
    return examples.find(ex) != examples.end();
}

bool common_arg::is_exclude(enum eden_example ex) {
...
bool common_arg::get_value_from_env(std::string & output) const {
    ...
    if (!args_neg.empty()) {
        // for compatibility, we need to check EDEN_ARG_NO_ env as well
        std::string neg_env = env;
        string_replace_all(neg_env, "EDEN_ARG_", "EDEN_ARG_NO_");
...
std::vector<std::string> common_arg::get_env() const {
    ...
    if (!args_neg.empty() && env) {
        // for compatibility, we need to add EDEN_ARG_NO_ variant
        std::string neg_env = env;
        string_replace_all(neg_env, "EDEN_ARG_", "EDEN_ARG_NO_");
...
static void parse_tensor_buffer_overrides(const std::string & value, std::vector<eden_model_tensor_buft_override> & overrides) {
...
static bool common_params_handle_remote_preset(common_params & params, eden_example ex) {
...
void common_params_handle_models(common_params & params, eden_example curr_ex) {
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    const bool skip_model_download = ctx_arg.ex == EDEN_EXAMPLE_EXPORT_GRAPH_OPS;
    ...
    if (params.model.path.empty() && ctx_arg.ex != EDEN_EXAMPLE_SERVER && !skip_model_download && !params.usage && !params.completion) {
    ...
    // pad tensor_buft_overrides for eden_params_fit:
    const size_t ntbo = eden_max_tensor_buft_overrides();
    ...
    if (!params.chat_template.empty() && !common_chat_verify_template(params.chat_template, params.use_jinja)) {
        throw std::runtime_error(string_format(
            ...
            params.use_jinja ? "" : "\nnote: eden.cpp was started without --jinja, we only support commonly used templates"
        ));
    }
...
static void common_params_print_usage(common_params_context & ctx_arg) {
    ...
    for (auto & opt : ctx_arg.options) {
        // in case multiple EDEN_EXAMPLE_* are set, we prioritize the EDEN_EXAMPLE_* matching current example
        if (opt.is_sampling) {
    ...
    // TODO: maybe convert enum eden_example to string
...
static void common_params_print_completion(common_params_context & ctx_arg) {
    ...
    std::set<std::string> executables = {
        ...
        "eden-cli",
        "eden-completion",
        "eden-convert-eden2c-to-ggml",
        "eden-cvector-generator",
        "eden-debug",
        "eden-diffusion-cli",
        "eden-embedding",
        "eden-eval-callback",
        ...
        "eden-qwen2vl-cli",
        "eden-retrieval",
        "eden-save-load-state",
        "eden-server",
        "eden-simple",
        "eden-simple-chat",
        "eden-speculative",
        "eden-speculative-simple",
        "eden-tokenize",
        "eden-tts",
        "eden-vdot"
    };
    ...
    for (const auto& exe : executables) {
        printf("complete -F _eden_completions %s\n", exe.c_str());
    }
}
...
bool common_params_to_map(int argc, char ** argv, eden_example ex, std::map<common_arg, std::string> & out_map) {
...
bool common_params_parse(int argc, char ** argv, common_params & params, eden_example ex, void(*print_usage)(int, char **)) {
...
static std::string list_builtin_chat_templates() {
    ...
    int32_t res = eden_chat_builtin_templates(nullptr, 0);
    supported_tmpl.resize(res);
    res = eden_chat_builtin_templates(supported_tmpl.data(), supported_tmpl.size());
    std::ostringstream msg;
...
common_params_context common_params_parser_init(common_params & params, eden_example ex, void(*print_usage)(int, char **)) {
    // per-example default params
    // we define here to make sure it's included in eden-gen-docs
    if (ex == EDEN_EXAMPLE_COMPLETION) {
        params.use_jinja = false;   // disable jinja by default

    } else if (ex == EDEN_EXAMPLE_MTMD) {
        params.use_jinja = false;   // disable jinja by default
        params.sampling.temp = 0.2; // lower temp by default for better quality

    } else if (ex == EDEN_EXAMPLE_SERVER) {
     ...
     * rules:
     * - all examples inherit options from EDEN_EXAMPLE_COMMON
     * - if EDEN_EXAMPLE_* is set (other than COMMON), we only show the option in the corresponding example
     * - if both {EDEN_EXAMPLE_COMMON, EDEN_EXAMPLE_*,} are set, we will prioritize the EDEN_EXAMPLE_* matching current example
     */
    auto add_opt = [&](common_arg arg) {
        if ((arg.in_example(ex) || arg.in_example(EDEN_EXAMPLE_COMMON)) && !arg.is_exclude(ex)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params &) {
            fprintf(stderr, "version: %d (%s)\n", eden_build_number(), eden_commit());
            fprintf(stderr, "built with %s for %s\n", eden_compiler(), eden_build_target());
            exit(0);
        }
    ));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        {"--completion-bash"},
        "print source-able bash completion script for eden.cpp",
        [](common_params & params) {
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_EMBEDDING, EDEN_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--display-prompt"},
        {"--no-display-prompt"},
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_LOOKUP}));
    add_opt(common_arg(
        {"-t", "--threads"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_LOOKUP, EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            params.n_ctx = value;
            if (value == 0) {
                // disable context reduction in eden_params_fit if the user explicitly requests the full context size:
                params.fit_params_min_ctx = UINT32_MAX;
            }
        }
    ).set_env("EDEN_ARG_CTX_SIZE"));
    add_opt(common_arg(
        {"-n", "--predict", "--n-predict"}, "N",
        string_format(
            ex == EDEN_EXAMPLE_COMPLETION
    ...
    add_opt(common_arg(
        ...
        string_format("use full-size SWA cache (default: %s)\n"
            "[(more info)](https://github.com/ggml-org/eden.cpp/pull/13194#issuecomment-2868343055)", params.swa_full ? "true" : "false"),
        [](common_params & params) {
            params.swa_full = true;
        }
    ).set_env("EDEN_ARG_SWA_FULL"));
    add_opt(common_arg(
        {"-ctxcp", "--ctx-checkpoints", "--swa-checkpoints"}, "N",
        string_format("max number of context checkpoints to create per slot (default: %d)"
            "[(more info)](https://github.com/ggml-org/eden.cpp/pull/15293)", params.n_ctx_checkpoints),
        [](common_params & params, int value) {
            params.n_ctx_checkpoints = value;
        }
    ).set_env("EDEN_ARG_CTX_CHECKPOINTS").set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_env("EDEN_ARG_CHECKPOINT_EVERY_NT").set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
        string_format("set the maximum cache size in MiB (default: %d, -1 - no limit, 0 - disable)"
            "[(more info)](https://github.com/ggml-org/eden.cpp/pull/16391)", params.cache_ram_mib),
        [](common_params & params, int value) {
            params.cache_ram_mib = value;
        }
    ).set_env("EDEN_ARG_CACHE_RAM").set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
    ...
    ).set_env("EDEN_ARG_KV_UNIFIED").set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_PERPLEXITY, EDEN_EXAMPLE_BATCHED, EDEN_EXAMPLE_BENCH, EDEN_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--cache-idle-slots"},
        {"--no-cache-idle-slots"},
    ...
    ).set_env("EDEN_ARG_CACHE_IDLE_SLOTS").set_examples({EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_IMATRIX, EDEN_EXAMPLE_PERPLEXITY}).set_env("EDEN_ARG_CONTEXT_SHIFT"));
    add_opt(common_arg(
        {"--chunks"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_IMATRIX, EDEN_EXAMPLE_PERPLEXITY, EDEN_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg({ "-fa", "--flash-attn" }, "[on|off|auto]",
                       string_format("set Flash Attention use ('on', 'off', or 'auto', default: '%s')",
                                     eden_flash_attn_type_name(params.flash_attn_type)),
                       [](common_params & params, const std::string & value) {
                           if (is_truthy(value)) {
                               ...
                               params.flash_attn_type = EDEN_FLASH_ATTN_TYPE_AUTO;
                           } else {
                               throw std::runtime_error(
                                   string_format("error: unknown value for --flash-attn: '%s'\n", value.c_str()));
                           }
                       }).set_env("EDEN_ARG_FLASH_ATTN"));
    add_opt(common_arg(
        {"-p", "--prompt"}, "PROMPT",
    ...
    ).set_excludes({EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_DIFFUSION, EDEN_EXAMPLE_MTMD}));
    add_opt(common_arg(
        {"--perf"},
        {"--no-perf"},
        string_format("whether to enable internal libeden performance timings (default: %s)", params.no_perf ? "true" : "false"),
        [](common_params & params, bool value) {
            params.no_perf = !value;
            params.sampling.no_perf = !value;
        }
    ).set_env("EDEN_ARG_PERF"));
    add_opt(common_arg(
        {"--show-timings"},
        {"--no-show-timings"},
    ...
    ).set_examples({EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SHOW_TIMINGS"));
    add_opt(common_arg(
        {"-f", "--file"}, "FNAME",
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_DIFFUSION}));
    add_opt(common_arg(
        {"--in-file"}, "FNAME",
    ...
    ).set_examples({EDEN_EXAMPLE_IMATRIX}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"--prompt-cache"}, "FNAME",
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-sp", "--special"},
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_MTMD, EDEN_EXAMPLE_EMBEDDING, EDEN_EXAMPLE_RETRIEVAL, EDEN_EXAMPLE_PERPLEXITY, EDEN_EXAMPLE_DEBUG}));
    add_opt(common_arg(
        {"--spm-infill"},
        string_format(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--samplers"}, "SAMPLERS",
    ...
    add_opt(common_arg(
        {"-s", "--seed"}, "SEED",
        string_format("RNG seed (default: %d, use random seed for %d)", params.sampling.seed, EDEN_DEFAULT_SEED),
        [](common_params & params, const std::string & value) {
    ...
    ).set_sampling().set_env("EDEN_ARG_TOP_K"));
    add_opt(common_arg(
        {"--top-p"}, "N",
    ...
    add_opt(common_arg(
        ...
        string_format("adaptive-p: select tokens near this probability (valid range 0.0 "
                      ...
                      "[(more info)](https://github.com/ggml-org/eden.cpp/pull/17927)",
    ...
    ).set_sampling().set_env("EDEN_ARG_BACKEND_SAMPLING"));
    add_opt(common_arg(
        {"--pooling"}, "{none,mean,cls,last,rank}",
        "pooling type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.pooling_type = EDEN_POOLING_TYPE_NONE; }
            else if (value == "mean") { params.pooling_type = EDEN_POOLING_TYPE_MEAN; }
            else if (value == "cls")  { params.pooling_type = EDEN_POOLING_TYPE_CLS;  }
            else if (value == "last") { params.pooling_type = EDEN_POOLING_TYPE_LAST; }
            else if (value == "rank") { params.pooling_type = EDEN_POOLING_TYPE_RANK; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({EDEN_EXAMPLE_EMBEDDING, EDEN_EXAMPLE_RETRIEVAL, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_DEBUG}).set_env("EDEN_ARG_POOLING"));
    add_opt(common_arg(
        {"--attention"}, "{causal,non-causal}",
        "attention type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "causal") { params.attention_type = EDEN_ATTENTION_TYPE_CAUSAL; }
            else if (value == "non-causal") { params.attention_type = EDEN_ATTENTION_TYPE_NON_CAUSAL; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({EDEN_EXAMPLE_EMBEDDING}));
    add_opt(common_arg(
        {"--rope-scaling"}, "{none,linear,yarn}",
        "RoPE frequency scaling method, defaults to linear unless specified by the model",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.rope_scaling_type = EDEN_ROPE_SCALING_TYPE_NONE; }
            else if (value == "linear") { params.rope_scaling_type = EDEN_ROPE_SCALING_TYPE_LINEAR; }
            else if (value == "yarn") { params.rope_scaling_type = EDEN_ROPE_SCALING_TYPE_YARN; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_env("EDEN_ARG_ROPE_SCALING_TYPE"));
    add_opt(common_arg(
        {"--rope-scale"}, "N",
    ...
    ).set_env("EDEN_ARG_ROPE_FREQ_BASE"));
    add_opt(common_arg(
        {"--rope-freq-scale"}, "N",
    ...
    ).set_env("EDEN_ARG_ROPE_FREQ_SCALE"));
    add_opt(common_arg(
        {"--yarn-orig-ctx"}, "N",
    ...
    ).set_env("EDEN_ARG_YARN_ORIG_CTX"));
    add_opt(common_arg(
        {"--yarn-ext-factor"}, "N",
    ...
    ).set_env("EDEN_ARG_YARN_EXT_FACTOR"));
    add_opt(common_arg(
        {"--yarn-attn-factor"}, "N",
    ...
    ).set_env("EDEN_ARG_YARN_ATTN_FACTOR"));
    add_opt(common_arg(
        {"--yarn-beta-slow"}, "N",
    ...
    ).set_env("EDEN_ARG_YARN_BETA_SLOW"));
    add_opt(common_arg(
        {"--yarn-beta-fast"}, "N",
    ...
    ).set_env("EDEN_ARG_YARN_BETA_FAST"));
    add_opt(common_arg(
        {"-gan", "--grp-attn-n"}, "N",
    ...
    ).set_env("EDEN_ARG_GRP_ATTN_N").set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
        {"-gaw", "--grp-attn-w"}, "N",
    ...
    ).set_env("EDEN_ARG_GRP_ATTN_W").set_examples({EDEN_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"-kvo", "--kv-offload"},
        {"-nkvo", "--no-kv-offload"},
    ...
    ).set_examples({EDEN_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--hellaswag-tasks"}, "N",
    ...
    ).set_env("EDEN_ARG_DEFRAG_THOLD"));
    if (ex == EDEN_EXAMPLE_SERVER) {
        ...
        ).set_env("EDEN_ARG_N_PARALLEL").set_examples({EDEN_EXAMPLE_SERVER}));
    } else {
        add_opt(common_arg(
            {"-np", "--parallel"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-cb", "--cont-batching"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_CONT_BATCHING"));
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_MMPROJ"));
    add_opt(common_arg(
        {"-mmu", "--mmproj-url"}, "URL",
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_MMPROJ_URL"));
    add_opt(common_arg(
        {"--mmproj-auto"},
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_MMPROJ_AUTO"));
    add_opt(common_arg(
        {"--mmproj-offload"},
        {"--no-mmproj-offload"},
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_MMPROJ_OFFLOAD"));
    add_opt(common_arg(
        {"--image", "--audio"}, "FILE",
    ...
    ).set_examples({EDEN_EXAMPLE_MTMD, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--image-min-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_IMAGE_MIN_TOKENS"));
    add_opt(common_arg(
        {"--image-max-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("EDEN_ARG_IMAGE_MAX_TOKENS"));
    if (eden_supports_rpc()) {
        add_opt(common_arg(
            {"--rpc"}, "SERVERS",
    ...
    add_opt(common_arg(
        ...
        "see https://github.com/ggml-org/eden.cpp/issues/1437",
        [](common_params & params, const std::string & value) {
    ...
    ).set_env("EDEN_ARG_OVERRIDE_TENSOR"));
    add_opt(common_arg(
        {"-cmoe", "--cpu-moe"},
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (!eden_supports_gpu_offload()) {
                ...
                fprintf(stderr, "warning: one possible reason is that eden.cpp was compiled without GPU support\n");
                fprintf(stderr, "warning: consult docs/build.md for compilation instructions\n");
            }
        }
    ).set_env("EDEN_ARG_N_GPU_LAYERS"));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            if (value == "none") {
                params.split_mode = EDEN_SPLIT_MODE_NONE;
            } else if (value == "layer") {
                params.split_mode = EDEN_SPLIT_MODE_LAYER;
            } else if (value == "row") {
                params.split_mode = EDEN_SPLIT_MODE_ROW;
            } else if (value == "tensor") {
                params.split_mode = EDEN_SPLIT_MODE_TENSOR;
            } else {
                throw std::invalid_argument("invalid value");
            }
            if (!eden_supports_gpu_offload()) {
                fprintf(stderr, "warning: eden.cpp was compiled without support for GPU offload. Setting the split mode has no effect.\n");
            }
        }
    ).set_env("EDEN_ARG_SPLIT_MODE"));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (split_arg.size() >= eden_max_devices()) {
                throw std::invalid_argument(
                    string_format("got %zu input configs, but system only has %zu devices", split_arg.size(), eden_max_devices())
                );
            }
            for (size_t i = 0; i < eden_max_devices(); ++i) {
                if (i < split_arg.size()) {
            ...
            if (!eden_supports_gpu_offload()) {
                fprintf(stderr, "warning: eden.cpp was compiled without support for GPU offload. Setting a tensor split has no effect.\n");
            }
        }
    ).set_env("EDEN_ARG_TENSOR_SPLIT"));
    add_opt(common_arg(
        {"-mg", "--main-gpu"}, "INDEX",
        ...
        [](common_params & params, int value) {
            ...
            if (!eden_supports_gpu_offload()) {
                fprintf(stderr, "warning: eden.cpp was compiled without support for GPU offload. Setting the main GPU has no effect.\n");
            }
        }
    ).set_env("EDEN_ARG_MAIN_GPU"));
    add_opt(common_arg(
        { "-fit", "--fit" }, "[on|off]",
    ...
    ).set_examples({EDEN_EXAMPLE_FIT_PARAMS}).set_env("EDEN_ARG_FIT_ESTIMATE"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMMON, EDEN_EXAMPLE_EXPORT_LORA}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_ALIAS"));
    add_opt(common_arg(
        {"--tags"}, "STRING",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_TAGS"));
    add_opt(common_arg(
        {"-m", "--model"}, "FNAME",
        ex == EDEN_EXAMPLE_EXPORT_LORA
            ? "model path from which to load base model"
            : "model path to load",
        [](common_params & params, const std::string & value) {
            params.model.path = value;
        }
    ).set_examples({EDEN_EXAMPLE_COMMON, EDEN_EXAMPLE_EXPORT_LORA}).set_env("EDEN_ARG_MODEL"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--chunk-size"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_PASSKEY, EDEN_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--pos"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_IMATRIX, EDEN_EXAMPLE_CVECTOR_GENERATOR, EDEN_EXAMPLE_EXPORT_LORA, EDEN_EXAMPLE_TTS, EDEN_EXAMPLE_FINETUNE,
                    EDEN_EXAMPLE_RESULTS, EDEN_EXAMPLE_EXPORT_GRAPH_OPS}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_BENCH, EDEN_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-tgs"},
    ...
    ).set_examples({EDEN_EXAMPLE_BENCH}));
    add_opt(common_arg(
        {"-ntg"}, "n0,n1,...",
    ...
    ).set_examples({EDEN_EXAMPLE_EMBEDDING, EDEN_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_HOST"));
    add_opt(common_arg(
        {"--port"}, "PORT",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_PORT"));
    add_opt(common_arg(
        {"--reuse-port"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_REUSE_PORT"));
    add_opt(common_arg(
        {"--path"}, "PATH",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_STATIC_PATH"));
    add_opt(common_arg(
        {"--api-prefix"}, "PREFIX",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_API_PREFIX"));
    add_opt(common_arg(
        {"--webui-config"}, "JSON",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_WEBUI_CONFIG"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_WEBUI_CONFIG_FILE"));
    add_opt(common_arg(
        {"--webui-mcp-proxy"},
        {"--no-webui-mcp-proxy"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_WEBUI_MCP_PROXY"));
    add_opt(common_arg(
        {"--tools"}, "TOOL1,TOOL2,...",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_TOOLS"));
    add_opt(common_arg(
        {"--webui"},
        {"--no-webui"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_WEBUI"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_DEBUG}).set_env("EDEN_ARG_EMBEDDINGS"));
    add_opt(common_arg(
        {"--rerank", "--reranking"},
        string_format("enable reranking endpoint on server (default: %s)", "disabled"),
        [](common_params & params) {
            params.embedding = true;
            params.pooling_type = EDEN_POOLING_TYPE_RANK;
        }
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_RERANKING"));
    add_opt(common_arg(
        {"--api-key"}, "KEY",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_API_KEY"));
    add_opt(common_arg(
        {"--api-key-file"}, "FNAME",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_SSL_KEY_FILE"));
    add_opt(common_arg(
        {"--ssl-cert-file"}, "FNAME",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_SSL_CERT_FILE"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_CHAT_TEMPLATE_KWARGS"));
    add_opt(common_arg(
        {"-to", "--timeout"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_TIMEOUT"));
    add_opt(common_arg(
        {"--threads-http"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_THREADS_HTTP"));
    add_opt(common_arg(
        {"--cache-prompt"},
        {"--no-cache-prompt"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_CACHE_PROMPT"));
    add_opt(common_arg(
        {"--cache-reuse"}, "N",
        string_format(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_CACHE_REUSE"));
    add_opt(common_arg(
        {"--metrics"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_ENDPOINT_METRICS"));
    add_opt(common_arg(
        {"--props"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_ENDPOINT_PROPS"));
    add_opt(common_arg(
        {"--slots"},
        {"--no-slots"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_ENDPOINT_SLOTS"));
    add_opt(common_arg(
        {"--slot-save-path"}, "PATH",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_MODELS_DIR"));
    add_opt(common_arg(
        {"--models-preset"}, "PATH",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_MODELS_PRESET"));
    add_opt(common_arg(
        {"--models-max"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_MODELS_MAX"));
    add_opt(common_arg(
        {"--models-autoload"},
        {"--no-models-autoload"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_MODELS_AUTOLOAD"));
    add_opt(common_arg(
        {"--jinja"},
        {"--no-jinja"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_MTMD}).set_env("EDEN_ARG_JINJA"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_THINK"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_REASONING"));
    add_opt(common_arg(
        {"--reasoning-budget"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_THINK_BUDGET"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_THINK_BUDGET_MESSAGE"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_MTMD}).set_env("EDEN_ARG_CHAT_TEMPLATE"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
        {"--skip-chat-parsing"},
    ...
    ).set_examples({EDEN_EXAMPLE_COMPLETION, EDEN_EXAMPLE_CLI, EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_SKIP_CHAT_PARSING"));
    add_opt(common_arg(
        {"--prefill-assistant"},
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER}).set_env("EDEN_ARG_PREFILL_ASSISTANT"));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_CVECTOR_GENERATOR}));
    add_opt(common_arg(
        {"--negative-file"}, "FNAME",
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_HF_REPO"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
    ...
    ).set_env("EDEN_ARG_SPEC_DRAFT_CACHE_TYPE_K"));
    add_opt(common_arg(
    ...
    ).set_env("EDEN_ARG_SPEC_DRAFT_CACHE_TYPE_V"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_CPU_MOE"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_N_CPU_MOE"));
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_LOOKUP, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_N_MAX"));
    add_opt(common_arg(
        {"--spec-draft-n-min"}, "N",
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_LOOKUP, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_N_MIN"));
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_P_SPLIT"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_P_MIN"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_N_GPU_LAYERS_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_DRAFT_MODEL"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_SPEC_TYPE"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_LOOKUP, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_DRAFT_MAX"));
    add_opt(common_arg(
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SPECULATIVE, EDEN_EXAMPLE_LOOKUP, EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}).set_env("EDEN_ARG_DRAFT_MIN"));
    add_opt(common_arg(
        {"--spec-ngram-size-n"}, "N",
    ...
    ).set_spec().set_examples({EDEN_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--spec-ngram-size-m"}, "N",
    ...
    ).set_examples({EDEN_EXAMPLE_TTS, EDEN_EXAMPLE_SERVER}));
     add_opt(common_arg(
    ...
    ).set_examples({ EDEN_EXAMPLE_DIFFUSION }));
    add_opt(common_arg(
        {"--diffusion-visual"},
    ...
    ).set_examples({ EDEN_EXAMPLE_FINETUNE }));
    ...
    ).set_examples({EDEN_EXAMPLE_RESULTS}));
    add_opt(common_arg(
        {"--save-logits"},
    ...
    ).set_examples({EDEN_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({EDEN_EXAMPLE_EMBEDDING, EDEN_EXAMPLE_SERVER}));
    ...
    ).set_examples({EDEN_EXAMPLE_SERVER, EDEN_EXAMPLE_CLI}));

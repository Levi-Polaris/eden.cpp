    ...
    switch (format.mode) {
        ...
        default:
            LOG_ERR("[ERROR] Template seems to support tool calls, but failed to determine tool format. Tool calling will not work properly. "
                "Check for a fixed template for your model in the models/templates directory of your eden.cpp installation or "
                "report an issue at https://github.com/ggml-org/eden.cpp/issues\n");
            return ctx.p.eps();
    }
}

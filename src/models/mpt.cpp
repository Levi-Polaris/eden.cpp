...
void eden_model_mpt::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_mpt::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // FIXME test-eden-archs crashes if q_norm is created
...
std::unique_ptr<llm_graph_context> eden_model_mpt::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_mpt::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {

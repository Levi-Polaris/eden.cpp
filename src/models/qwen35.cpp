...
void eden_model_qwen35::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_qwen35::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_qwen35::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_qwen35::graph::graph(const eden_model & model, const llm_graph_params & params) :
...
std::pair<ggml_tensor *, ggml_tensor *> eden_model_qwen35::graph::build_qkvz(
                ggml_tensor * input,
                        int   il) {
...
ggml_tensor * eden_model_qwen35::graph::build_norm_gated(
        ggml_tensor * input,
        ggml_tensor * weights,
        ggml_tensor * gate,
        int           layer) {
...
ggml_tensor * eden_model_qwen35::graph::build_layer_attn(
...
ggml_tensor * eden_model_qwen35::graph::build_layer_attn_linear(
...
ggml_tensor * eden_model_qwen35::graph::build_layer_ffn(ggml_tensor * cur, const int il) {

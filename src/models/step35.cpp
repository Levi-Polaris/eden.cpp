...
void eden_model_step35::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_step35::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // optional rope factors (eden3) / longrope tensors
...
std::unique_ptr<llm_graph_context> eden_model_step35::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_step35::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ggml_tensor * cur;
    ggml_tensor * inpL;

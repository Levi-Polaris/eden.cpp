...
void eden_model_gemma3n::load_arch_hparams(eden_model_loader & ml) {
    uint32_t swa_period = 5;
    ml.get_key_or_arr(LLM_KV_ATTENTION_SLIDING_WINDOW_PATTERN, swa_period, false);
    hparams.swa_type = EDEN_SWA_TYPE_STANDARD;
...
void eden_model_gemma3n::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_gemma3n::build_arch_graph(const llm_graph_params & params) const {
...
eden_model_gemma3n::graph::graph(const eden_model & model, const llm_graph_params & params) :
...
ggml_tensor * eden_model_gemma3n::graph::calc_magnitude(ggml_tensor * x) {
    return ggml_sqrt(ctx0, ggml_sum_rows(ctx0, ggml_sqr(ctx0, x)));
}

// equivalent to get_per_layer_inputs() in python code
// output shape: [n_embd_altup, n_layer, n_tokens]
ggml_tensor * eden_model_gemma3n::graph::build_inp_per_layer() {
...
ggml_tensor * eden_model_gemma3n::graph::project_per_layer_inputs(ggml_tensor * inp_batch, ggml_tensor * inp_per_layer) {
...
ggml_tensor * eden_model_gemma3n::graph::laurel(ggml_tensor * cur, int il) {
    ggml_tensor * tmp = cur;
...
ggml_tensor * eden_model_gemma3n::graph::gaussian_topk(ggml_tensor * x) {
...
// input x shape: [n_embd,  n_tokens]
// output  shape: [n_altup, n_tokens]
ggml_tensor * eden_model_gemma3n::graph::altup_compute_router_modalities(ggml_tensor * x, int il) {
...
ggml_tensor * eden_model_gemma3n::graph::altup_predict(ggml_tensor * cur, int il) {
...
// output                  shape: [n_embd, n_tokens, n_altup]
ggml_tensor * eden_model_gemma3n::graph::altup_correct(ggml_tensor * predictions, ggml_tensor * activated, int il) {
    ggml_tensor * modalities = altup_compute_router_modalities(activated, il);  // [n_altup, n_tokens]
    cb(modalities, "modalities", il);

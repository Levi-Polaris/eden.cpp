...
void eden_model_openai_moe::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_openai_moe::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_openai_moe::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_openai_moe::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ggml_tensor * cur;
    ggml_tensor * inpL;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        cur = build_moe_ffn(cur,
                ...
                EDEN_EXPERT_GATING_FUNC_TYPE_SOFTMAX_WEIGHT,
                il);

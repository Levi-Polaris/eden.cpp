...
void eden_model_lfm2::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_lfm2::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_lfm2::build_arch_graph(const llm_graph_params & params) const {
    if (hparams.swa_type == EDEN_SWA_TYPE_STANDARD) {
...
template <bool iswa>
eden_model_lfm2::graph<iswa>::graph(const eden_model & model, const llm_graph_params & params) :
    ...
    using mem_hybrid_ctx  = std::conditional_t<iswa, eden_memory_hybrid_iswa_context, eden_memory_hybrid_context>;
    ...
    auto build_moe_feed_forward = [&model, this](ggml_tensor * cur, int il) -> ggml_tensor * {
        return build_moe_ffn(cur,
                ...
                static_cast<eden_expert_gating_func_type>(hparams.expert_gating_func),
                il);
    };
...
template struct eden_model_lfm2::graph<true>;
template struct eden_model_lfm2::graph<false>;

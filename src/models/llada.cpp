...
void eden_model_llada::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_llada::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // Use separate Q, K, V projections without bias, matching LLaDAEdenBlock
        layer.wq =
...
std::unique_ptr<llm_graph_context> eden_model_llada::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_llada::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            // compute separate Q, K, V projections without bias, matching LLaDAEdenBlock

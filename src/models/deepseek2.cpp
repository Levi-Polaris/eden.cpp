...
void eden_model_deepseek2::load_arch_hparams(eden_model_loader & ml) {
    uint32_t n_vocab = 0;
    ...
    if (hparams.expert_gating_func == EDEN_EXPERT_GATING_FUNC_TYPE_NONE) {
        // for compatibility with existing DeepSeek V2 and V2.5 GGUFs
        // that have no expert_gating_func model parameter set
        if ((hparams.n_layer == 47 || hparams.n_layer == 48) && n_vocab == 154880) {
            // GLM 4.7 Lite
            hparams.expert_gating_func = EDEN_EXPERT_GATING_FUNC_TYPE_SIGMOID;
        } else {
            hparams.expert_gating_func = EDEN_EXPERT_GATING_FUNC_TYPE_SOFTMAX;
        }
    }
...
void eden_model_deepseek2::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_deepseek2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_deepseek2::graph::graph(const eden_model & model, const llm_graph_params & params) :
    ...
    // See https://github.com/ggml-org/eden.cpp/discussions/7416 for detailed explanation.
    // And also: https://github.com/ggml-org/eden.cpp/pull/17945 [TAG_DEEPSEEK2_YARN_LOG_MUL_FIX]

    // first cancel the adjustment from eden_hparams::yarn_attn_factor_adjust to get the original attn_factor
    ...
    for (int il = 0; il < effective_n_layers; ++il) {
        ...
        if (is_ocr) {
            ...
            if (is_mla) {
                ...
                if (inp_attn_scale) {
                    // apply eden 4 temperature scaling
        ...
        if ((uint32_t) il < hparams.n_layer_dense_lead) {
            ...
            ggml_tensor * moe_out = build_moe_ffn(cur,
                ...
                (eden_expert_gating_func_type) hparams.expert_gating_func,
                il,
                nullptr,

...
void eden_model_plamo2::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_plamo2::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_plamo2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_plamo2::graph::graph(const eden_model & model, const llm_graph_params & params) :
...
ggml_tensor * eden_model_plamo2::graph::build_plamo2_attn_layer(llm_graph_input_attn_kv * inp,
                                                        ggml_tensor *             inp_pos,
                                                        ggml_tensor *             cur,
...
ggml_tensor * eden_model_plamo2::graph::build_plamo2_mamba_layer(llm_graph_input_rs * inp,
                                                         ggml_tensor *        cur,
                                                         const eden_model &  model,

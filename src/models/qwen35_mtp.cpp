...
void eden_model_qwen35_mtp::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_qwen35_mtp::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_qwen35_mtp::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

// LLM_ARCH_QWEN35_MTP draft head for Qwen35-6 series
eden_model_qwen35_mtp::graph::graph(const eden_model & model, const llm_graph_params & params)
...
void eden_model_qwen35_mtp::link_shared_tensors(const eden_model * main_model) {
    ...
    if (main_embd) {
        EDEN_LOG_INFO("%s: sharing tok_embd from trunk (%s, ne=[%d %d %d %d]) — saving ~682 MiB\n",
        ...
        EDEN_LOG_WARN("%s: trunk model has no token_embd.weight or output.weight\n", __func__);
    }
    if (main_output_norm) {
        output_norm = const_cast<ggml_tensor *>(main_output_norm);
    } else {
        EDEN_LOG_WARN("%s: trunk model has no output_norm.weight\n", __func__);
    }
}

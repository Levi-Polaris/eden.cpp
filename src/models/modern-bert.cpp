...
void eden_model_modern_bert::load_arch_hparams(eden_model_loader & ml) {
    const bool found_swa = ml.get_key(LLM_KV_ATTENTION_SLIDING_WINDOW, hparams.n_swa, false);
    if (found_swa && hparams.n_swa > 0) {
        hparams.swa_type = EDEN_SWA_TYPE_SYMMETRIC;
...
void eden_model_modern_bert::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_modern_bert::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_modern_bert::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {

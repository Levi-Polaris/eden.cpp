...
void eden_model_rwkv6qwen2::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_rwkv6qwen2::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_rwkv6qwen2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

eden_model_rwkv6qwen2::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_build_rwkv6_base(model, params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        const eden_layer * layer = &model.layers[il];

...
void eden_model_jais2::load_arch_hparams(eden_model_loader & ml) {
...
void eden_model_jais2::load_arch_tensors(eden_model_loader &) {
    EDEN_LOAD_LOCALS;
...
std::unique_ptr<llm_graph_context> eden_model_jais2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

// JAIS-2 model graph builder
// Uses: LayerNorm (not RMSNorm), relu2 activation, separate Q/K/V, RoPE embeddings
eden_model_jais2::graph::graph(const eden_model & model, const llm_graph_params & params) : llm_graph_context(params) {

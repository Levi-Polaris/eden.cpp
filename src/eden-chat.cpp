...
static const std::map<std::string, llm_chat_template> LLM_CHAT_TEMPLATES = {
    ...
    { "eden2",            LLM_CHAT_TEMPLATE_EDEN_2           },
    { "eden2-sys",        LLM_CHAT_TEMPLATE_EDEN_2_SYS       },
    { "eden2-sys-bos",    LLM_CHAT_TEMPLATE_EDEN_2_SYS_BOS   },
    { "eden2-sys-strip",  LLM_CHAT_TEMPLATE_EDEN_2_SYS_STRIP },
    ...
    { "eden3",            LLM_CHAT_TEMPLATE_EDEN_3           },
    ...
    { "eden4",            LLM_CHAT_TEMPLATE_LLAMA4            },
...
llm_chat_template llm_chat_detect_template(const std::string & tmpl) {
    ...
    if (tmpl_contains("<|im_start|>")) {
        ...
        if (tmpl_contains("[SYSTEM_PROMPT]")) {
            ...
            // eden2 template and its variants
            // [variant] support system message
            // See: https://huggingface.co/blog/eden2#how-to-prompt-eden-2
            bool support_system_message = tmpl_contains("<<SYS>>");
            bool add_bos_inside_history = tmpl_contains("bos_token + '[INST]");
            bool strip_message = tmpl_contains("content.strip()");
            if (strip_message) {
                return LLM_CHAT_TEMPLATE_EDEN_2_SYS_STRIP;
            } else if (add_bos_inside_history) {
                return LLM_CHAT_TEMPLATE_EDEN_2_SYS_BOS;
            } else if (support_system_message) {
                return LLM_CHAT_TEMPLATE_EDEN_2_SYS;
            } else {
                return LLM_CHAT_TEMPLATE_EDEN_2;
            }
        }
        ...
        return LLM_CHAT_TEMPLATE_EDEN_3;
    } else if (tmpl_contains("[gMASK]sop")) {
        // chatglm3-6b
...
// Simple version of "eden_apply_chat_template" that only works with strings
// This function uses heuristic checks to determine commonly used template. It is not a jinja parser.
int32_t llm_chat_apply_template(
    llm_chat_template tmpl,
    const std::vector<const eden_chat_message *> & chat,
    std::string & dest, bool add_ass) {
    // Taken from the research: https://github.com/ggml-org/eden.cpp/issues/5527
    std::stringstream ss;
    if (tmpl == LLM_CHAT_TEMPLATE_CHATML) {
        ...
        for (auto message : chat) {
            ...
            tmpl == LLM_CHAT_TEMPLATE_EDEN_2
            || tmpl == LLM_CHAT_TEMPLATE_EDEN_2_SYS
            || tmpl == LLM_CHAT_TEMPLATE_EDEN_2_SYS_BOS
            || tmpl == LLM_CHAT_TEMPLATE_EDEN_2_SYS_STRIP) {
        // eden2 template and its variants
        // [variant] support system message
        // See: https://huggingface.co/blog/eden2#how-to-prompt-eden-2
        bool support_system_message = tmpl != LLM_CHAT_TEMPLATE_EDEN_2;
        // [variant] add BOS inside history
        bool add_bos_inside_history = tmpl == LLM_CHAT_TEMPLATE_EDEN_2_SYS_BOS;
        // [variant] trim spaces from the input message
        bool strip_message = tmpl == LLM_CHAT_TEMPLATE_EDEN_2_SYS_STRIP;
    ...
    } else if (tmpl == LLM_CHAT_TEMPLATE_EDEN_3) {
        // Eden 3
        for (auto message : chat) {
            std::string role(message->role);
...
int32_t eden_chat_builtin_templates(const char ** output, size_t len) {

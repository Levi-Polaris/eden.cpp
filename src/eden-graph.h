...
class eden_kv_cache_context;
class eden_kv_cache_iswa_context;
class eden_memory_recurrent_context;
class eden_memory_hybrid_context;
class eden_memory_hybrid_iswa_context;
...
struct eden_cross {
    ...
    //       ref: https://github.com/ggml-org/eden.cpp/pull/11213#discussion_r1969892524
    ...
    std::vector<std::set<eden_seq_id>> seq_ids_enc;
};
...
public:
    llm_graph_input_i() {
        const char * EDEN_GRAPH_INPUT_DEBUG = getenv("EDEN_GRAPH_INPUT_DEBUG");
        debug = EDEN_GRAPH_INPUT_DEBUG ? atoi(EDEN_GRAPH_INPUT_DEBUG) : 0;
    }

    virtual ~llm_graph_input_i() = default;

    virtual void set_input(const eden_ubatch * ubatch) = 0;
...
// temperature tuning, used by eden4
...
public:
    llm_graph_input_pos_bucket(const eden_hparams & hparams) : hparams(hparams) {}
...
public:
    llm_graph_input_pos_bucket_kv(
            const eden_hparams & hparams,
            const eden_kv_cache_context * mctx) : hparams(hparams), mctx(mctx) {}
...
public:
    llm_graph_input_mean(const eden_cparams & cparams) : cparams(cparams) {}
...
public:
    llm_graph_input_cls(const eden_cparams & cparams, const llm_arch arch) : cparams(cparams), arch(arch) {}
...
public:
    llm_graph_input_rs(const eden_memory_recurrent_context * mctx) : mctx(mctx) {}
    ...
    const eden_memory_recurrent_context * mctx;
...
public:
    llm_graph_input_cross_embd(
            const eden_cross * cross) : cross(cross) {}
    ...
    const eden_cross * cross;
};

class llm_graph_input_attn_no_cache : public llm_graph_input_i {
public:
    llm_graph_input_attn_no_cache(const eden_hparams & hparams, const eden_cparams & cparams) :
        hparams(hparams),
        cparams(cparams) {
    }
...
// ref: https://github.com/ggml-org/eden.cpp/pull/19067
...
public:
    llm_graph_input_attn_kv_iswa(
            const eden_hparams & hparams,
            const eden_cparams & cparams,
            const eden_kv_cache_iswa_context * mctx) :
        hparams(hparams),
        cparams(cparams),
        mctx(mctx) {
    }
...
public:
    llm_graph_input_attn_cross(const eden_cross * cross) : cross(cross) {}
    ...
    const eden_cross * cross = nullptr;
};
...
public:
    llm_graph_input_mem_hybrid(
            ...
            const eden_memory_hybrid_context *      mctx) :
...
public:
    llm_graph_input_mem_hybrid_iswa(
            ...
            std::unique_ptr<llm_graph_input_rs>          inp_rs,
            const eden_memory_hybrid_iswa_context *     mctx) :
        inp_attn(std::move(inp_attn)),
        inp_rs(std::move(inp_rs)),
        cparams(cparams),
        mctx(mctx) { }
...
public:
    llm_graph_input_sampling(std::map<eden_seq_id, eden_sampler *> samplers) :
...
// these objects deliver the result from the graph build process back to the eden_context
// note that the input tensors created for the graph are referenced here - the goal is to be able to populate their
//   specific data, by calling the set_inputs() method
// along with the input tensors, the object also provides commonly used outputs tensors, such as logits, embeddings, etc.
//   these are used by the eden_context to extact the relevant data, based on the compute parameters

// callback that allows us to apply custom logic to each tensor (e.g. ggml-alloc, offloading, etc.)
using llm_graph_cb = std::function<void(const eden_ubatch & ubatch, ggml_tensor * cur, const char * name, int il)>;
...
struct llm_graph_params {
    ...
    eden_ubatch ubatch; // note: intentionally make a copy
    ...
    const eden_adapter_cvec     * cvec;
    const eden_adapter_loras    * loras;
    ...
    static bool samplers_equal(
          const std::map<eden_seq_id, eden_sampler *> & lhs,
          const std::map<eden_seq_id, eden_sampler *> & rhs) {
...
public:
    ...
    void set_inputs(const eden_ubatch * ubatch);
    void set_outputs();
    ...
    std::map<eden_seq_id, ggml_tensor*> t_sampled_logits;
    std::map<eden_seq_id, ggml_tensor*> t_candidates;
    std::map<eden_seq_id, ggml_tensor*> t_sampled;
    std::map<eden_seq_id, ggml_tensor*> t_sampled_probs;
...
struct llm_graph_context {
    ...
    const enum eden_pooling_type pooling_type;
    const enum eden_rope_type    rope_type;
    ...
    // TODO: move this implementation to eden_memory_recurrent.
    //       this is analogous to eden_kv_cache::cpy_k / cpy_v

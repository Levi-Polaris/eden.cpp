...
enum llm_type {
    ...
    LLM_TYPE_57B_A14B,
    LLM_TYPE_17B_16E, // eden4 Scout
    LLM_TYPE_17B_128E, // eden4 Maverick
    LLM_TYPE_A13B,
    LLM_TYPE_7B_A1B,
...
struct eden_layer {
    ...
    struct eden_layer_posnet posnet;

    struct eden_layer_convnext convnext;

    struct eden_layer_shortconv shortconv;

    struct eden_layer_nextn nextn;
};
...
struct eden_meta_device_get_split_state_userdata {
    size_t                     n_devices;
...
struct eden_model {
    ...
    std::vector<eden_device> devices;

    // for quantize-stats only
    std::vector<std::pair<std::string, struct ggml_tensor *>> tensors_by_name;

    // for keeping track of associated LoRA adapters
    std::unordered_set<eden_adapter_lora *> loras;

    // statically allocated context for assigning
    struct eden_meta_device_get_split_state_userdata get_split_state_ud;

    int64_t t_load_us  = 0;
    int64_t t_start_us = 0;

    explicit eden_model(const eden_model_params & params);
    virtual ~eden_model();
    ...
    eden_split_mode split_mode() const;
    ...
    virtual void link_shared_tensors(const eden_model * main_model) { (void)main_model; }

    float get_rope_freq_base (const eden_cparams & cparams, int il) const;
    float get_rope_freq_scale(const eden_cparams & cparams, int il) const;

    ggml_tensor * get_rope_factors(const eden_cparams & cparams, int il) const;

    eden_memory_i * create_memory(const eden_memory_params & params, const eden_cparams & cparams) const;

    ggml_cgraph * build_graph(const llm_graph_params & params) const;

    virtual void load_stats  (eden_model_loader & ml) = 0;
    virtual void load_hparams(eden_model_loader & ml) = 0;
    virtual void load_vocab  (eden_model_loader & ml) = 0;
    virtual bool load_tensors(eden_model_loader & ml) = 0; // returns false if cancelled by progress_callback

    // model must define these
    virtual void load_arch_hparams(eden_model_loader & ml) = 0;
    virtual void load_arch_tensors(eden_model_loader & ml) = 0;
    virtual std::unique_ptr<llm_graph_context> build_arch_graph(const llm_graph_params & params) const = 0;

protected:
    eden_model_params params;
...
struct eden_model_base : public eden_model {
    friend struct eden_model;

    eden_model * model;
    eden_model_loader * ml = nullptr;
    const LLM_TN tn;

    // eden_model_loader is not yet defined at this point, so we will set it after construction
    const int TENSOR_DUPLICATED;
    const int TENSOR_NOT_REQUIRED;
    const int TENSOR_SKIP;
    const int TENSOR_SKIP_IF_VIRTUAL;

    explicit eden_model_base(const eden_model_params & params);
    virtual ~eden_model_base() = default;

    ggml_tensor * create_tensor(eden_model_loader & ml, const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags);

    // convenience overload of create_tensor that doesn't require eden_model_loader
    ...
    void create_tensor_gate_up_exps(eden_layer & layer, int bid, int64_t n_embd_,
                int64_t n_ff_, int64_t n_expert_, int flags);

    // helper: try to load merged qkv first, fall back to separate q, k, v
    void create_tensor_qkv(eden_layer & layer, int bid,
                int64_t n_embd_, int64_t n_embd_q_, int64_t n_embd_k_, int64_t n_embd_v_,
                int flags);

    void load_stats  (eden_model_loader & ml) override;
    void load_hparams(eden_model_loader & ml) override;
    void load_vocab  (eden_model_loader & ml) override;
    bool load_tensors(eden_model_loader & ml) override;

    // model must define these
    void load_arch_hparams(eden_model_loader & ml) override = 0;
    void load_arch_tensors(eden_model_loader & ml) override = 0;
    std::unique_ptr<llm_graph_context> build_arch_graph(const llm_graph_params & params) const override = 0;
};

const char * llm_type_name(llm_type type);

// convenience macro for loading local variables for load_tensors() in eden_model_base
// note: cast to int64_t since we will use these for the tensor dimensions
#define EDEN_LOAD_LOCALS \
